...
from data import CONFIG, OBJ_ASSETS, CHAR_ASSETS, UI_FONT_FILES, iter_music_assets, music_dir, music_file_relpath
try:
    ...
    def resolve_asset_dir(rel_dir):
        ...
        return None
from flow import (
    condition_step_chain_mode,
    evaluate_event_step_condition,
    evaluate_global_condition,
)
...
_DERIVED_CHAR_ANIM_CACHE = OrderedDict()  # (char, state, direction) -> [Surface, ...]
# 파생 프레임 시각 Y 오프셋(원본 px). 양수=화면 아래. 발(하단) 앵커라 픽셀 패딩으로 처리 불가 → blit 시 적용.
# WeakKeyDictionary 로 Surface→값 보관 (id() 재사용 시 오적용 방지).
_DERIVED_SURF_DRAW_OFFSET_Y = WeakKeyDictionary()  # Surface -> int
_SURF_UID_MAP = WeakKeyDictionary()  # Surface -> int (스케일 캐시 키)
_SURF_UID_SEQ = 0
_IMG_CACHE_MAX = 2048
_ANIM_CACHE_MAX = 512
_DERIVED_CHAR_ANIM_CACHE_MAX = 256


def _ensure_surf_uid(surf) -> int:
    """Surface 수명 동안 고유한 정수. 스케일 캐시 키에 id(surf) 대신 사용."""
    global _SURF_UID_SEQ
    if surf is None:
        return 0
    try:
        uid = _SURF_UID_MAP.get(surf)
        if uid is not None:
            return int(uid)
    except Exception:
        pass
    _SURF_UID_SEQ += 1
    uid = int(_SURF_UID_SEQ)
    try:
        _SURF_UID_MAP[surf] = uid
        return uid
    except Exception:
        return int(id(surf))
...
def _clamp_wall_angle_deg(v):
    """맵 스프라이트 벽 각도(도). 0=맵 +X(1-2-3), 90=맵 +Y(1-4-7). [0, 360)."""
    try:
        a = float(v)
    except (TypeError, ValueError):
        return 0.0
...
def field_wall_3d_flag(v) -> bool:
    """
    world_data/object_defs 의 wall_3d.
    True면 wall_angle이 0°(+X)여도 벽 컬럼(3D_ROTATE 열 원근)을 쓴다.
    기본 False — 모든 오브젝트에 열 렌더를 켜면 비용이 커서 인스턴스마다 지정.
    """
    if v is True:
        return True
...
def field_wall_angle_active(angle_deg, force=False) -> bool:
    """벽 컬럼 렌더: 각도가 0°가 아니거나 wall_3d(force)가 True."""
    if force:
        return True
    a = _clamp_wall_angle_deg(angle_deg)
    return min(a, 360.0 - a) > 0.05


def field_item_is_wall(obj) -> bool:
    """FieldItem 벽 컬럼 여부. wall_3d 또는 wall_angle≠0."""
    if obj is None:
        return False
    force = field_wall_3d_flag(getattr(obj, "wall_3d", False))
    return field_wall_angle_active(getattr(obj, "wall_angle", 0.0), force=force)


def field_wall_dir_xy(angle_deg):
    rad = math.radians(_clamp_wall_angle_deg(angle_deg))
    return math.cos(rad), math.sin(rad)


def field_wall_native_half_w(img_w) -> float:
    try:
        w = float(img_w)
    except (TypeError, ValueError):
        return 0.0
...
def field_wall_along_world(pos_x, pos_y, along, angle_deg):
    c, s = field_wall_dir_xy(angle_deg)
    return float(pos_x) + float(along) * c, float(pos_y) + float(along) * s


def field_wall_world_aabb(pos_x, pos_y, img_w, img_h, angle_deg, *, height=0.0):
    """
    에디터 픽/컬링용 AABB.
    세로 픽셀은 화면 위(=에디터 월드 -Y)로 올라간다고 본다(기존 빌보드 rect와 동일 관습).
    """
    ...
    xs, ys = [], []
    for sign in (-1.0, 1.0):
        fx = float(pos_x) + sign * half * c
        fy = float(pos_y) + sign * half * s - h_off
        xs.extend((fx, fx))
        ys.extend((fy, fy - ih))
    if not xs:
        return pygame.Rect(int(pos_x), int(pos_y), 1, 1)
    ...
    return pygame.Rect(
        ...
        max(1, int(math.ceil(bot - top))),
    )


def field_wall_editor_hit(pos_x, pos_y, img_w, img_h, angle_deg, wx, wy, *, height=0.0, tol=3.0) -> bool:
    """플랫 에디터 월드 좌표 기준 벽 히트(컬럼 기하)."""
    half = field_wall_native_half_w(img_w)
    try:
        ih = float(img_h)
    except (TypeError, ValueError):
    ...
    try:
        ...
        h_off = 0.0
    c, s = field_wall_dir_xy(angle_deg)
    dx = float(wx) - float(pos_x)
    dy = float(wy) - (float(pos_y) - h_off)
    along = dx * c + dy * s
    t = float(tol)
    if along < -half - t or along > half + t:
        return False
    fx = float(pos_x) + along * c
    fy = (float(pos_y) - h_off) + along * s
    if float(wy) < fy - ih - t or float(wy) > fy + t:
        return False
    return abs(float(wx) - fx) <= t + 0.75
...
def get_cached_scaled_sprite(image, eff_zoom: float, sprite_perspective_q=None, sprite_tilt: float = 1.0):
    ...
    try:
        ...
        use_smooth = False
    key = ("spr", _ensure_surf_uid(image), sw, sh, pq, st, 1 if use_smooth else 0)
...
def blit_change_ghost_at_feet(
    ...
    *,
    ...
    """
    CHANGE 크로스페이드: 교체 직전 스프라이트를 발 앵커로 겹쳐 그린다.
    ghost = {"img": Surface, "alpha": 0~255, "eff_z": 배율(없으면 zoom)}.
    """
    if screen is None or not isinstance(ghost, dict):
        return
    img = ghost.get("img")
    if img is None:
        return
    try:
        a = int(ghost.get("alpha", 0) or 0)
    except (TypeError, ValueError):
        a = 0
    if a <= 0:
        return
    try:
        gz = float(ghost.get("eff_z", zoom) or zoom)
    except (TypeError, ValueError):
        gz = float(zoom)
    if gz <= 1e-6:
        return
    render = get_cached_scaled_sprite(
        img,
        gz,
    ...
    if a < 255:
        try:
            render = render.copy()
            render.set_alpha(max(0, min(255, a)))
        except Exception:
            pass
    fpx, fpy = int(round(float(feet_x))), int(round(float(feet_y)))
    dx, dy = blit_topleft_bottom_center(
        fpx, fpy, render.get_width(), render.get_height()
    )
    try:
        screen.blit(render, (dx, dy))
    except Exception:
        pass


def snapshot_change_ghost(entity):
    """CHANGE 직전 프레임을 복사해 크로스페이드 ghost dict 로 남긴다."""
    img = getattr(entity, "image", None)
    if img is None:
        return None
    try:
        snap = img.copy()
        ...
        snap = img
    try:
        a = int(getattr(entity, "alpha", 255) or 255)
    except (TypeError, ValueError):
        a = 255
    return {"img": snap, "alpha": max(0, min(255, a))}


def _clear_entity_change_ghost(entity):
    if entity is None:
        return
    try:
        entity._change_ghost = None
    except Exception:
        pass
...
def _scan_music_folder(rel_dir):
    """폴더 안의 mp3/ogg/wav → {파일명(확장자 제외): 경로}."""
    out = {}
    base_dir = resolve_asset_dir(rel_dir)
    if not base_dir:
        return out
    try:
...
def _scan_music_library():
    """
    BGM 라이브러리 {id: abs_path}.
    1) data.MUSIC_ASSETS 등록 키 (이벤트 music 필드)
    2) 파일명 스템 별칭 (예전 이벤트·한글 파일명 호환)
    3) MUSIC_INCLUDE_UNREGISTERED 이면 폴더에만 있는 곡도 파일명으로
    """
    out = {}
    try:
        include_unreg = bool(CONFIG.get("MUSIC_INCLUDE_UNREGISTERED", True))
    except Exception:
        include_unreg = True

    for mid, filename, _title in iter_music_assets():
        rel = music_file_relpath(filename)
        if not rel:
            continue
        ...
        if not ok:
            continue
        out[mid] = path
        stem = os.path.splitext(os.path.basename(str(filename).replace("\\", "/")))[0]
        if stem and stem not in out:
            out[stem] = path

    if include_unreg:
        for rel in (music_dir(), os.path.join("assets", "music")):
            for name, path in _scan_music_folder(rel).items():
                if name not in out:
                    out[name] = path
    return out
...
def flip_vertical_top_shear_plan(plan, shear_px):
    """정상 쉬어 계획(위→오른쪽)을 리버스(위→왼쪽)로 뒤집기.

    정상: top dx=shear_px, bottom dx=0
    리버스: top dx=0, bottom dx=shear_px
    blit_x=bg 이면 위가 왼쪽으로 보임. blit_x=bg-shear(뷰포트 확장)여도 상대 방향은 동일.
    스프라이트 x_offset 은 field_runtime.shear_screen_x_offset 로 blit 방식에 맞춰 계산.
    """
    if not plan:
        return plan
    try:
    ...
    for item in plan:
        try:
            yy, hh, dxs = item
            out.append((int(yy), int(hh), int(spx) - int(dxs)))
        except Exception:
            return plan
    return tuple(out)
...
class MusicManager:
    ...
    트랙 ID는 data.MUSIC_ASSETS (assets/musics). MUSIC_PLAY music 필드와 동일.
    요구 기능:
    - 곡 시작 페이드인
    - 곡 종료/전환 페이드아웃(명령 기반)
    - 일시정지/재개
    ...
    def __init__(self):
        ...
        self._paused = False
        self._volume = 1.0  # 최근 의도 볼륨 (뮤트 해제 시 복구)
        self._user_muted = False  # 설정창 "음악 끄고켜기" — 재생은 유지·볼륨만 0
        # 소프트웨어 페이드인 — SDL Mix_FadeInMusic 이 포맷/플랫폼에서 무시되는 경우가 있어
        # update() 에서 set_volume 보간. {"t0_ms", "dur_ms", "target"}
        self._fade_in = None
        try:
            n = len(self.library)
            if n <= 0:
    ...
    def _clear_fade_in(self):
        self._fade_in = None

    def _fade_in_progress_01(self):
        """진행 중 페이드인 비율 0~1. 없으면 None."""
        fi = self._fade_in
        if not fi:
            return None
        try:
            dur = max(1, int(fi.get("dur_ms") or 1))
            elapsed = int(pygame.time.get_ticks()) - int(fi.get("t0_ms") or 0)
            return max(0.0, min(1.0, float(elapsed) / float(dur)))
        except Exception:
            return 1.0

    def _intended_play_volume(self):
        """뮤트 전 의도 출력 볼륨(페이드인 진행 반영)."""
        u = self._fade_in_progress_01()
        target = float(self._volume)
        fi = self._fade_in
        if fi is not None:
            try:
                target = float(fi.get("target", target))
            except (TypeError, ValueError):
                pass
        if u is None:
            return max(0.0, min(1.0, target))
        return max(0.0, min(1.0, target * float(u)))

    def _apply_out_volume(self, vol=None):
        """실제 mixer 볼륨 적용. vol=None 이면 의도 볼륨(+뮤트) 사용."""
        try:
            if vol is None:
                base = self._intended_play_volume()
            else:
                base = max(0.0, min(1.0, float(vol)))
            out = 0.0 if self._user_muted else base
            pygame.mixer.music.set_volume(max(0.0, min(1.0, out)))
        except Exception:
            pass
    ...
    def _load_and_play(self, name, fade_in_ms=0, loop=False, volume=None):
        ...
        try:
            ...
            # 볼륨 0에서 시작해 update() 보간 — SDL fade_ms 에 의존하지 않음
            if fade_ms > 0:
                self._fade_in = {
                    "t0_ms": int(pygame.time.get_ticks()),
                    "dur_ms": fade_ms,
                    "target": float(self._volume),
                }
                self._apply_out_volume(0.0)
            else:
                self._clear_fade_in()
                self._apply_out_volume(float(self._volume))
    ...
    def play_if_changed(self, name, fade_in_ms=400, loop=True, volume=None):
        """같은 곡이 이미 재생 중이면 건드리지 않음. 맵 기본 BGM·이벤트 종료 복귀에 사용."""
        if not name:
            return False
        try:
            same = str(self.current_name or "") == str(name)
            busy = bool(pygame.mixer.music.get_busy()) and not self._paused
        except Exception:
            same, busy = False, False
        if same and busy:
            return True
        if same and self._paused:
            self.resume()
            return True
        return self.play(
            name,
    ...
    def play(self, name, fade_in_ms=0, loop=False, volume=None, queue_after_current=True):
        """
        - queue_after_current=True: 지금 곡이 재생 중이면 끝난 뒤 재생
    ...
    def is_user_muted(self) -> bool:
        """설정창에서 끈 BGM(볼륨 0) 상태."""
        return bool(self._user_muted)

    def set_user_muted(self, muted: bool) -> bool:
        """True=음악 끔(볼륨 0), False=이전 볼륨으로 켬. 반환=현재 뮤트 여부."""
        want = bool(muted)
        if want == bool(self._user_muted):
            return bool(self._user_muted)
        if want:
            # 페이드인 중이면 중간 볼륨으로 _volume 을 덮지 않음(타깃 유지)
            if self._fade_in is None:
                try:
                    cur = float(pygame.mixer.music.get_volume())
                    if cur > 1e-6:
                        self._volume = max(0.0, min(1.0, cur))
                except Exception:
                    pass
            ...
            self._apply_out_volume()
        return bool(self._user_muted)

    def toggle_user_muted(self) -> bool:
        """뮤트 토글. 반환=토글 후 뮤트 여부."""
        return self.set_user_muted(not bool(self._user_muted))

    def update(self):
        """매 프레임: 페이드인 볼륨 보간 + 큐된 곡 시작."""
        fi = self._fade_in
        if fi is not None:
            u = self._fade_in_progress_01()
            if u is None or u >= 1.0:
                try:
                    self._volume = float(fi.get("target", self._volume))
                except (TypeError, ValueError):
                    pass
                self._clear_fade_in()
                self._apply_out_volume(float(self._volume))
                ...
                try:
                    target = float(fi.get("target", self._volume))
                except (TypeError, ValueError):
                    target = float(self._volume)
                self._apply_out_volume(target * float(u))
        if not self._queued:
            return
        try:
            busy = bool(pygame.mixer.music.get_busy())
...
def _blit_feet_shadow(
    ...
    """
    발 그림자. Mode7 중에는 지면 좌표를 rotate3d_mode7_project 로 투영한다.
    shadow_angle_deg가 양수면 화면에서 반시계 방향, 즉 타원의 오른쪽이 위로 기운다.
    """
    ...
    base_rx = float(CONFIG.get("SHADOW_ELLIPSE_RX", 12))
    base_ry = float(CONFIG.get("SHADOW_ELLIPSE_RY", 4))
    ...
    if a < 6:
        return
    try:
        shadow_angle_deg = max(-45.0, min(45.0, float(shadow_angle_deg)))
    except (TypeError, ValueError):
        shadow_angle_deg = 0.0
    angle_key = round(shadow_angle_deg, 2)
    # 성능: 발그림자는 엔티티 수만큼 호출되므로 매 프레임 Surface 생성/ellipse draw를 피한다.
    # (rx,ry,color,alpha,angle)별로 작게 캐시.
    global _FEET_SHADOW_CACHE
    try:
        _FEET_SHADOW_CACHE
    except NameError:
        _FEET_SHADOW_CACHE = {}
    k = (int(rx), int(ry), int(r), int(g), int(b), int(a), angle_key)
    ...
    if surf is None:
        ...
        if abs(shadow_angle_deg) >= 0.01:
            surf = pygame.transform.rotate(surf, shadow_angle_deg)
...
def _char_anim_dir_exists(char_name, state, direction="left") -> bool:
    path = os.path.join("assets", "images", "character", char_name, f"{state}_{direction}")
    return os.path.isdir(path)


def _load_char_anim_frames_disk(char_name, state, direction="left"):
    """디스크 폴더만 로드 (파생/idle 폴백 없음). 없으면 None."""
    path = os.path.join("assets", "images", "character", char_name, f"{state}_{direction}")
    return _load_char_anim_dir_cached(path, state, direction, char_name)


def _rotate_cw90_align_feet(src: pygame.Surface) -> pygame.Surface:
    """
    CW90 회전 후, 원본 이미지의 하단 중앙점과
    회전된 이미지의 하단 중앙점을 일치시키도록 캔버스에 배치한다.

    원본 발(하단 중앙): (w//2, h-1)
    회전 후 이미지 크기: rw=h, rh=w  →  회전 이미지 하단 중앙: (rw//2, rh-1)

    두 점을 맞추려면:
      - 회전 이미지를 캔버스에 blit할 때
        원본 발이 있던 절대 좌표에 회전 이미지의 하단 중앙이 오도록 배치.
    캔버스 크기는 두 이미지를 모두 포함할 수 있도록 충분히 잡고,
    원본 발 위치를 기준점으로 공유한다.
    """
    if src is None:
        return src
    try:
        base = src.convert_alpha()
    except Exception:
        base = src
    ...
    rot = pygame.transform.rotate(base, -90)
    rw, rh = rot.get_width(), rot.get_height()  # rw=h, rh=w
    ...
    # 회전 이미지를 blit할 위치:
    # 캔버스 내 foot_x, foot_y 위치에 회전 이미지의 하단 중앙(r_foot_x, r_foot_y)이 오도록
    blit_x = foot_x - r_foot_x
    blit_y = foot_y - r_foot_y

    # 캔버스 크기: 회전 이미지 전체가 들어오도록
    canvas_x0 = min(0, blit_x)
    canvas_y0 = min(0, blit_y)
    canvas_x1 = max(w, blit_x + rw)
    canvas_y1 = max(h, blit_y + rh)
    out_w = max(1, canvas_x1 - canvas_x0)
    out_h = max(1, canvas_y1 - canvas_y0)
    ...
    out.blit(rot, (blit_x, blit_y))
    return out


def _set_derived_surf_draw_offset_y(surf, offset_y) -> None:
    """파생 프레임에 blit Y 오프셋 기록. 양수=화면 아래(발 앵커 기준)."""
    if surf is None:
        return
    try:
        oy = int(offset_y or 0)
    except (TypeError, ValueError):
        oy = 0
    try:
        if oy == 0:
            _DERIVED_SURF_DRAW_OFFSET_Y.pop(surf, None)
        else:
            _DERIVED_SURF_DRAW_OFFSET_Y[surf] = oy
    except Exception:
        pass


def get_derived_surf_draw_offset_y(surf) -> int:
    if surf is None:
        return 0
    try:
        return int(_DERIVED_SURF_DRAW_OFFSET_Y.get(surf, 0) or 0)
    except (TypeError, ValueError):
        return 0


def _flip_anim_frames_keep_derived_offset(frames):
    """좌→우 flip. 파생 offset_y 메타데이터도 새 Surface에 복사."""
    out = []
    for img in frames or []:
        try:
            ...
            flipped = img
        oy = get_derived_surf_draw_offset_y(img)
        if oy:
            _set_derived_surf_draw_offset_y(flipped, oy)
        out.append(flipped)
    return out


def _apply_derived_frame_ops(surf: pygame.Surface, *, rotate_cw_deg=0, align_feet=True, offset_y=0) -> pygame.Surface:
    if surf is None:
        return surf
    try:
        deg = int(rotate_cw_deg or 0) % 360
    except (TypeError, ValueError):
        deg = 0
    out = surf
    if deg == 90:
        if align_feet:
            out = _rotate_cw90_align_feet(out)
        else:
            try:
                out = pygame.transform.rotate(out.convert_alpha(), -90)
            except Exception:
                out = pygame.transform.rotate(out, -90)
    elif deg == 180:
        try:
            out = pygame.transform.rotate(out.convert_alpha(), 180)
        except Exception:
            out = pygame.transform.rotate(out, 180)
        if align_feet:
            pass
    elif deg == 270:
        if align_feet:
            ...
            try:
                ...
                out = pygame.transform.rotate(out, 90)
    # offset_y: 발(하단) 앵커라 상단 패딩은 시각 위치가 안 바뀜 → blit 시 적용할 메타만 기록
    try:
        oy = int(offset_y or 0)
    except (TypeError, ValueError):
        oy = 0
    if oy != 0 and out is not None:
        _set_derived_surf_draw_offset_y(out, oy)
    return out


def _pick_source_frame(frames, index):
    if not frames:
        return None
    try:
        ix = int(index)
...
def _derived_anim_recipes():
    try:
        from data import CHAR_DERIVED_ANIM_SETS

        raw = CHAR_DERIVED_ANIM_SETS
    except Exception:
        raw = None
    return raw if isinstance(raw, dict) else {}


def list_char_anim_set_names():
    """에디터/이벤트용: 기본 세트 + 가상(파생) 세트 이름."""
    base = [
        "idle",
        "walk",
        "jump",
    ...
    for k in _derived_anim_recipes().keys():
        ks = str(k or "").strip().lower()
        if ks and ks not in base:
            base.append(ks)
    return base


def _load_derived_char_anim(char_name, state, direction="left"):
    """
    CHAR_DERIVED_ANIM_SETS 레시피로 프레임 생성.
    디스크에 state 폴더가 있으면 None (호출측에서 폴더 우선).
    """
    name = str(char_name or "").strip()
    st = str(state or "").strip().lower()
    direc = str(direction or "left").strip().lower() or "left"
    if not name or not st:
        return None
    if _char_anim_dir_exists(name, st, direc):
        return None
    recipes = _derived_anim_recipes()
    recipe = recipes.get(st)
    if not isinstance(recipe, dict):
        return None

    cache_key = (name, st, direc)
    try:
        cache_key = (
            name,
            st,
            direc,
            int(recipe.get("offset_y") or 0),
            bool(recipe.get("align_feet", True)),
            int(recipe.get("rotate_cw_deg") or 0),
        )
    except (TypeError, ValueError):
        cache_key = (name, st, direc)
    cached = _DERIVED_CHAR_ANIM_CACHE.get(cache_key)
    if cached is not None:
        try:
            _DERIVED_CHAR_ANIM_CACHE.move_to_end(cache_key)
        except Exception:
            pass
        return cached

    out_frames = []
    frame_specs = recipe.get("frames")
    if isinstance(frame_specs, (list, tuple)) and frame_specs:
        for spec in frame_specs:
            if not isinstance(spec, dict):
                continue
            src_st = str(spec.get("source") or spec.get("from") or "idle").strip().lower()
            src_frames = _load_char_anim_frames_disk(name, src_st, direc)
            if not src_frames:
                # run 없으면 walk 등 — load_anim_auto 폴백은 순환 주의, disk만
                if src_st == "run":
                    src_frames = _load_char_anim_frames_disk(name, "walk", direc)
                if not src_frames and src_st != "idle":
                    src_frames = _load_char_anim_frames_disk(name, "idle", direc)
            if not src_frames:
                continue
            idx = spec.get("index", spec.get("frame", 0))
            fr = _pick_source_frame(src_frames, idx)
            if fr is None:
                continue
            fr = _apply_derived_frame_ops(
                fr,
                rotate_cw_deg=spec.get("rotate_cw_deg", recipe.get("rotate_cw_deg", 0)),
                align_feet=bool(spec.get("align_feet", recipe.get("align_feet", True))),
                offset_y=spec.get("offset_y", recipe.get("offset_y", 0)),
            )
            if fr is not None:
                out_frames.append(fr)
    else:
        src_st = str(
            recipe.get("source_set") or recipe.get("source") or recipe.get("from") or ""
        ).strip().lower()
        if not src_st:
            return None
        src_frames = _load_char_anim_frames_disk(name, src_st, direc)
        if not src_frames and src_st == "run":
            src_frames = _load_char_anim_frames_disk(name, "walk", direc)
        if not src_frames:
            return None
        rot = recipe.get("rotate_cw_deg", 0)
        align = bool(recipe.get("align_feet", True))
        oy = recipe.get("offset_y", 0)
        for fr in src_frames:
            out_frames.append(_apply_derived_frame_ops(fr, rotate_cw_deg=rot, align_feet=align, offset_y=oy))

    if not out_frames:
        return None
    _DERIVED_CHAR_ANIM_CACHE[cache_key] = out_frames
    try:
        _DERIVED_CHAR_ANIM_CACHE.move_to_end(cache_key)
    except Exception:
        pass
    while len(_DERIVED_CHAR_ANIM_CACHE) > _DERIVED_CHAR_ANIM_CACHE_MAX:
        try:
            _DERIVED_CHAR_ANIM_CACHE.popitem(last=False)
        except Exception:
            break
    return out_frames
...
def load_anim_auto(char_name, state, direction):
    ...
    if frames:
        return frames
    # 가상(파생) 세트 — falldown / tickle 등
    derived = _load_derived_char_anim(char_name, state, direction)
    if derived:
        return list(derived)
...
def load_racing_overlay_frames(anim_set: str = "moveinchworm_racing", facing: str = "left"):
    """
    공용 레이싱 탈것 오버레이 — assets/images/character/racing/<anim_set>_<facing>/
    몸(seat_idle) 뒤에 붙는 자벌레 등. 캐릭터별 폴더가 아니라 racing/ 공유.
    anim_set: moveinchworm_racing
    facing:
      front — 직진(뒤에서보기 기본)
      left  — 왼쪽 차선으로 옮기는 중. 오른쪽 이동은 이 세트를 좌우 반전해서 씀.
    front 폴더가 없으면 left 로 폴백.
    """
    st = str(anim_set or "moveinchworm_racing").strip().lower()
    if not st:
        return []
    face = str(facing or "left").strip().lower()
    if face not in ("left", "front"):
        face = "left"
    path = os.path.join("assets", "images", "character", "racing", f"{st}_{face}")
    frames = _load_anim_dir_cached(path)
    if not frames and face == "front":
...
def _load_char_state_or_fallback(char_name, state, fallback="walk"):
    ...
    derived = _load_derived_char_anim(char_name, state, "left")
    if derived:
        return list(derived)
...
class BaseCharacter:
    def __init__(self, name, pos, info):
        ...
        for st in ("hurt", "laugh", "attack", "lie", "seat", "seating", "seat_idle", "question", "surprise", "say", "sleep", "sad", "falldown", "tickle"):
            fb = _fallback_by_state.get(st, "idle")
            self.anims_l[st] = _load_char_state_or_fallback(name, st, fb)
        self.anims_r = {
            k: _flip_anim_frames_keep_derived_offset(v) for k, v in self.anims_l.items()
        }
    ...
    def retarget_char_def(self, new_name: str) -> bool:
        ...
        self.anims_r = {
            k: _flip_anim_frames_keep_derived_offset(v)
    ...
    def _apply_anim_override_teardown(self, ao):
        ...
        if rel == "stop":
            self.path = []
            self.target = list(self.pos)
            # 점프 중 release:stop → 공중 pos 잔여 없이 지면으로 복구
            clr = getattr(self, "_clear_jump_arc", None)
            if callable(clr):
                try:
                    clr(restore_pos=True)
                except Exception:
                    pass
            elif hasattr(self, "_jump_arc"):
                self._jump_arc = None
                try:
                    self._jump_draw_lift = 0.0
                except Exception:
                    pass
    ...
    def current_sprite_overlay_image(self):
        ...
        img = frames[idx]
        # flip_x 키가 있으면 엔티티 direction 대신 오버레이가 직접 좌우를 정한다
        # (레이싱 자벌레: 오른쪽 차선 이동 때 left 세트를 반전).
        if "flip_x" in ov:
            do_flip = bool(ov.get("flip_x"))
        else:
            do_flip = str(getattr(self, "direction", "left") or "left") == "right"
        if do_flip:
            try:
    ...
    def update_anim(self):
        ...
        st_key = str(active_state or "idle").strip().lower()
        try:
            delay_ms = int(CONFIG.get("ANIM_DELAY", 150) or 150)
        except Exception:
            delay_ms = 150
        if st_key == "jump":
            try:
                delay_ms = int(CONFIG.get("JUMP_ANIM_DELAY", 280) or 280)
            except Exception:
                delay_ms = 280
        delay_ms = max(16, int(delay_ms))
        if now - self.last_anim_time <= delay_ms:
            return
        ...
        if not frames:
            return
        # 상태 전환 시 이전 세트 프레임이 한 장 남는 현상 방지
        prev_st = getattr(self, "_anim_state_key", None)
        if prev_st != st_key:
            self._anim_state_key = st_key
            self.frame_idx = 0
            self.image = frames[0]
    ...
    def _draw_feet_shadow(
        ...
        """
        발밑 타원 그림자.
        - is_visible=False / alpha≈0 이면 그리지 않음 (PLACE remove·TUNE visible·페이드 퇴장과 맞춤)
        - alpha_scale_mul: 캐릭터 alpha(0~1) 등 추가 배율 — 페이드 중 그림자도 같이 옅어지게
        """
        if not CONFIG.get("CHARACTER_SHADOW_ENABLED", True):
            return
        # 숨김/페이드아웃된 캐릭터는 몸과 같이 그림자도 숨김
        if not bool(getattr(self, "is_visible", True)):
            return
        if bool(getattr(self, "_hide_feet_shadow", False)):
            return
        try:
            ent_a = int(getattr(self, "alpha", 255))
        except (TypeError, ValueError):
            ent_a = 255
        if ent_a <= 0:
            return
        ...
        # 캐릭터 alpha·호출측 배율 반영 (255 미만이면 그림자도 동일 비율로 옅어짐)
        try:
            a_mul = max(0.0, float(alpha_scale_mul))
        except (TypeError, ValueError):
            a_mul = 1.0
        if ent_a < 255:
            a_mul *= float(ent_a) / 255.0
        alpha_scale *= a_mul
        if alpha_scale < 0.02:
            return
        _blit_feet_shadow(
    ...
    def draw(self, screen, cam_x, cam_y, zoom=1.0, jump_shadow_mode=None, y_transform=None, x_offset_fn=None, sprite_perspective_q=None, shear_lod=False, x_scale_fn=None, x_shift_fn=None, pivot_xy=None, cam_angle_rad=0.0, mode7_ctx=None, view_w=None):
        # is_visible=False(TUNE/PLAYER_VISIBLE/PLACE remove) 또는 alpha=0(페이드 퇴장 완료)이면
        # 스프라이트·발그림자 모두 그리지 않음.
        ...
        try:
            ...
            a = 255
        ghost = getattr(self, "_change_ghost", None)
        if a <= 0 and not ghost:
            return
        ...
        # height(고정 띄우기) + 점프 포물선 연출(_jump_draw_lift). 월드는 발 좌표 유지.
        h_off = float(getattr(self, "height", 0) or 0)
        try:
            h_off += float(getattr(self, "_jump_draw_lift", 0.0) or 0.0)
        except (TypeError, ValueError):
            pass
        mode7_pr = None
        ...
        # 레이싱 탈것 underlay(hide_shadow) 탑승 중 · 미니게임(_hide_feet_shadow)에서는 그리지 않는다.
        # is_visible/alpha 는 _draw_feet_shadow 내부에서도 재확인.
        ov = getattr(self, "_sprite_overlay", None)
        hide_shadow = (
            isinstance(ov, dict) and ov.get("behind") and bool(ov.get("hide_shadow"))
        ) or bool(getattr(self, "_hide_feet_shadow", False))
        if not hide_shadow:
            self._draw_feet_shadow(
                ...
                entity_scale_mul=(
                    ez
                    * (float(mode7_pr.get("scale", 1.0)) if mode7_pr else 1.0)
                ),
                mode7_ctx=mode7_ctx,
        ...
        # 언더레이 프레임별 몸 상승량:
        # 자벌레처럼 굴곡하는 탈것은 발점(탈것 위치)은 고정하고 몸 이미지만 위로 움직인다.
        # 원본 스프라이트 기준 px 값이므로 현재 줌·Mode7 원근 배율(eff)을 함께 적용한다.
        body_lift_px = 0.0
        if isinstance(ov, dict) and ov.get("behind"):
            ...
            if isinstance(lift_profile, (list, tuple)) and lift_profile:
                try:
                    ...
                    body_lift_px = 0.0
        # 파생 애니 offset_y: 양수=아래로 (발 위치는 유지, 이미지만 이동)
        derived_oy = float(get_derived_surf_draw_offset_y(getattr(self, "image", None)))
        body_fpy = int(round(float(fpy) - body_lift_px * float(eff) + derived_oy * float(eff)))
        dx, dy = blit_topleft_bottom_center(
            fpx,
            body_fpy,
            render_img.get_width(),
        ...
        # 언더레이(behind): 원래 발점에 먼저 그림 — 레이싱 자벌레에 캐릭터가 앉은 연출
        if isinstance(ov, dict) and ov.get("behind"):
        ...
        if self.held_item and bool(getattr(self.held_item, "is_held", False)):
            ...
            try:
                carry_lift = float(getattr(self, "_jump_draw_lift", 0.0) or 0.0)
            except (TypeError, ValueError):
                carry_lift = 0.0
            try:
                carry_h = float(getattr(hi, "height", 0) or 0) + carry_lift
            except (TypeError, ValueError):
                carry_h = carry_lift
...
class MaskWalkingCharacter(BaseCharacter):
    ...
    def __init__(self, name, pos, info):
        ...
        self.jump_pad_zones = []
        self._jump_arc = None
        # 점프 연출 높이(px). pos 를 올리지 않고 draw 의 height_off 에만 가산.
        # (예전엔 pos[1]-=sin*arc_h 라서 arc 취소 시 한 프레임~영구로 ~JUMP_ARC_HEIGHT 만큼 어긋남)
        self._jump_draw_lift = 0.0
        ...
        self._follow_target = None  # (x,y) or None — reconnect A* 목표
        self._follow_slot_goal = None
        self._follow_last_plan_ms = 0
        # FOLLOW 트레일: 리더는 _follow_trail 에 발자국, 팔로워는 _follow_next_id 로 따라감
        self._follow_trail = None  # [(sid, x, y, tag), ...] tag 0=걷기 1=점프착지
        self._follow_trail_sid = 0
        self._follow_trail_gen = 0
        self._follow_trail_was_jump = False
        self._follow_next_id = 0
        self._follow_trail_seen_gen = None
        self._follow_reconnect_ms = 0
    ...
    def ground_feet_position(self):
        # pos 는 항상 지면 발 좌표 (점프 연출은 _jump_draw_lift)
        return float(self.pos[0]), float(self.pos[1])
    ...
    def jump_air_fraction(self):
        ...
        if ja is None:
            return None
        arc_h = float(ja.get("arc_h", CONFIG.get("JUMP_ARC_HEIGHT", 22)) or 0)
        if arc_h < 1e-6:
            return 0.0
        try:
            lift = float(getattr(self, "_jump_draw_lift", 0.0) or 0.0)
        except (TypeError, ValueError):
            lift = 0.0
        return max(0.0, min(1.0, lift / arc_h))

    def _clear_jump_arc(self, restore_pos=True):
        """
        _jump_arc 해제. restore_pos=True 이면 포물선 지면 트랙(또는 착지점)으로 pos 복구.
        예전 pos 보간 방식 잔여·중도 취소 시 ~arc_h 만큼 떠 있는 버그 방지.
        """
        ja = self._jump_arc
        if ja is not None and restore_pos:
            try:
                now = pygame.time.get_ticks()
                u = (now - float(ja["t0"])) / max(1.0, float(ja["dur"]))
            except Exception:
                u = 1.0
            try:
                if u >= 1.0:
                    self.pos = [float(ja["ex"]), float(ja["ey"])]
                else:
                    sm = _smoothstep_unit(u)
                    self.pos[0] = float(ja["sx"]) + (float(ja["ex"]) - float(ja["sx"])) * sm
                    self.pos[1] = float(ja["sy"]) + (float(ja["ey"]) - float(ja["sy"])) * sm
            except Exception:
                try:
                    self.pos = [float(ja.get("ex", self.pos[0])), float(ja.get("ey", self.pos[1]))]
                except Exception:
                    pass
        self._jump_arc = None
        self._jump_draw_lift = 0.0
    ...
    def move(self, mask_img, objects, npcs=None):
        ...
        if not self.path:
            self.state = "idle"
            # path 소진 시 arc 만 지우면 예전엔 공중 pos 가 남음 → 지면 복구
            if self._jump_arc is not None:
                self._clear_jump_arc(restore_pos=True)
            return
        ...
        if jump_wp:
            ...
            if u >= 1.0:
                ex, ey = float(ja["ex"]), float(ja["ey"])
                walkable, nl = self.check_walkable(ex, ey, mask_img, objects, npcs)
                if not walkable and mask_img is not None:
                    ...
                    if sn is not None:
                        ex, ey = float(sn[0]), float(sn[1])
                        walkable, nl = self.check_walkable(ex, ey, mask_img, objects, npcs)
                    if not walkable:
                        # 착지 불가면 점프 시작점으로 복귀
                        ex, ey = float(ja["sx"]), float(ja["sy"])
                        walkable, nl = self.check_walkable(ex, ey, mask_img, objects, npcs)
                self.pos = [ex, ey]
                self.path.pop(0)
                self._jump_arc = None
                ...
                if walkable:
                    self.layer = nl
                # ACTION_ANIM jump loop — 착지 후 제자리 재점프
                if getattr(self, "_hop_repeat", None):
                    if self._restart_hop_if_repeating():
                        return
    ...
    def follow_trail_clear(self):
        """리더 발자국 트레일 리셋. 맵 전환·순간이동·동행 해제 시."""
        self._follow_trail = []
        self._follow_trail_was_jump = False
        try:
            self._follow_trail_gen = int(getattr(self, "_follow_trail_gen", 0) or 0) + 1
        except Exception:
            self._follow_trail_gen = 1

    def follow_trail_sample(self):
        """
        리더가 실제로 서 있는 지면 좌표를 트레일에 남긴다.
        시간은 쓰지 않고 SAMPLE_PX 이동마다 1점 — FPS/부하와 무관하게 경로 모양이 같다.
        점프 시작 시 착지점을 tag=1 로 넣어 팔로워가 같은 도랑을 넘게 한다.
        """
        try:
            lx, ly = float(self.pos[0]), float(self.pos[1])
        except Exception:
            return
        trail = getattr(self, "_follow_trail", None)
        if trail is None:
            trail = []
            self._follow_trail = trail
        try:
            sample_px = float(CONFIG.get("FOLLOW_TRAIL_SAMPLE_PX", 10) or 10)
        except Exception:
            sample_px = 10.0
        sample_px = max(4.0, sample_px)
        try:
            max_n = int(CONFIG.get("FOLLOW_TRAIL_MAX_POINTS", 240) or 240)
        except Exception:
            max_n = 240
        max_n = max(16, max_n)
        try:
            break_px = float(CONFIG.get("FOLLOW_TRAIL_BREAK_PX", 96) or 96)
        except Exception:
            break_px = 96.0

        def _append(x, y, tag):
            buf = getattr(self, "_follow_trail", None)
            if buf is None:
                buf = []
                self._follow_trail = buf
            sid = int(getattr(self, "_follow_trail_sid", 0) or 0)
            buf.append((sid, float(x), float(y), int(tag)))
            self._follow_trail_sid = sid + 1
            while len(buf) > max_n:
                buf.pop(0)

        jumping = getattr(self, "_jump_arc", None) is not None
        was = bool(getattr(self, "_follow_trail_was_jump", False))
        if jumping and not was:
            ja = getattr(self, "_jump_arc", None) or {}
            try:
                _append(float(ja.get("ex", lx)), float(ja.get("ey", ly)), 1)
            except Exception:
                _append(lx, ly, 1)
            self._follow_trail_was_jump = True
            return
        if jumping:
            self._follow_trail_was_jump = True
            return
        ...
        if not trail:
            _append(lx, ly, 0)
            return
        _sid, px, py, _tg = trail[-1]
        gap = math.hypot(lx - float(px), ly - float(py))
        if gap >= break_px:
            self.follow_trail_clear()
            _append(lx, ly, 0)
            return
        if gap < sample_px:
            return
    ...
    def follow_step(self, leader_pos, desired_dist, mask_img, objects, npcs, speed_mul=1.0, leader=None):
        """
        FOLLOW: 리더 발자국 트레일을 따라 이동. 평소 A* 없음.
        홀드 직전 몇 점만 path 로 걸어 등속 이동. 홀드에서 멀면 바로 붙음(옛 트레일 되밟기 금지).
        """
        try:
        ...
        try:
            dist = float(desired_dist or 0)
        except Exception:
            dist = 0.0
        try:
            slot_arrive = float(CONFIG.get("FOLLOW_SLOT_ARRIVE_PX", 12) or 12)
        except Exception:
            slot_arrive = 12.0
        try:
            on_pt = float(CONFIG.get("FOLLOW_TRAIL_ARRIVE_PX", 2.5) or 2.5)
        except Exception:
            on_pt = 2.5
        on_pt = max(1.5, min(4.0, on_pt))
        try:
            reconnect_px = float(CONFIG.get("FOLLOW_TRAIL_RECONNECT_PX", 48) or 48)
        except Exception:
            reconnect_px = 48.0
        try:
            reconnect_ms = int(CONFIG.get("FOLLOW_TRAIL_RECONNECT_MS", 400) or 400)
        except Exception:
            reconnect_ms = 400
        try:
            look_n = int(CONFIG.get("FOLLOW_TRAIL_LOOKAHEAD", 6) or 6)
        except Exception:
            look_n = 6
        look_n = max(2, min(16, look_n))

        d_lead = math.hypot(lx - sx, ly - sy)
        trail = None
        if leader is not None:
            trail = getattr(leader, "_follow_trail", None)
        ...
        def _radial_slot():
            if d_lead < 1e-6:
                return (sx, sy)
            ux, uy = (lx - sx) / d_lead, (ly - sy) / d_lead
            tx = lx - ux * dist
            ty = ly - uy * dist
            try:
        ...
        def _hold_i():
            ...
            i = n - 1
            while i > 0:
                _a, x0, y0, _t0 = trail[i]
                _b, x1, y1, _t1 = trail[i - 1]
                acc += math.hypot(float(x0) - float(x1), float(y0) - float(y1))
                if acc >= dist:
                    return i - 1
                i -= 1
            return 0
        ...
        def _index_for_id(next_id):
            for i, pt in enumerate(trail):
                if int(pt[0]) >= int(next_id):
                    return i
            return len(trail) - 1 if trail else None

        def _pts_from(i0, i1):
            out = []
            hi = min(len(trail), int(i1) + 1)
            for k in range(max(0, int(i0)), hi):
                _sid, x, y, tg = trail[k]
                out.append((float(x), float(y), int(tg)))
            return out

        def _set_trail_polyline(pts):
            """남은 트레일을 path 에 이어 넣는다. move()가 CHAR_SPEED 로 등속 이동.
            첫 점이 같으면 현재 구간을 유지하고 뒤만 갱신(매 프레임 리셋하면 덜컥임)."""
            if not pts:
                return
            path = getattr(self, "path", None) or []
            if path:
                pr = path[0]
                if math.hypot(float(pr[0]) - float(pts[0][0]), float(pr[1]) - float(pts[0][1])) <= 1.5:
                    self.path = [pr] + list(pts[1:])
                    last = self.path[-1]
                    self.target = [float(last[0]), float(last[1])]
                    return
            self._path_plan_job = None
            self.path = list(pts)
            last = pts[-1]
            self.target = [float(last[0]), float(last[1])]
            if abs(float(pts[0][0]) - sx) > 0.1:
                self.direction = "left" if float(pts[0][0]) < sx else "right"
            if int(pts[0][2]) != 1:
                self.state = "run" if str(getattr(self, "_move_mode", "walk")) == "run" else "walk"

        def _reconnect_to(tx, ty, *, force=False):
            now_ms = pygame.time.get_ticks()
            if not force and getattr(self, "_path_plan_job", None) is not None:
                return
            last = int(getattr(self, "_follow_reconnect_ms", 0) or 0)
            if not force and (now_ms - last) < reconnect_ms and (getattr(self, "path", None) or []):
                return
            self._follow_reconnect_ms = int(now_ms)
            self._follow_target = (float(tx), float(ty))
            if mask_img:
        ...
        if self._jump_arc is not None:
            return
        path = getattr(self, "path", None) or []
        # 지금 점프 중인 웨이포인트만 건드리지 않음 (뒤에 점프가 있어도 앞 구간은 갱신)
        if path and len(path[0]) >= 3 and int(path[0][2]) == 1:
            return

        # 트레일이 짧으면(리더가 아직 dist 만큼 안 움직임) 사거리 안에서는 대기
        short_trail = (not trail) or (_trail_arclen() < dist * 0.5)
        if short_trail and d_lead <= dist + slot_arrive:
            if getattr(self, "_path_plan_job", None) or path:
        ...
        if short_trail:
            tx, ty = _radial_slot()
            if math.hypot(tx - sx, ty - sy) <= slot_arrive:
                return
            _reconnect_to(tx, ty)
            return
        ...
        if hold_i is None:
            return
        hx, hy = float(trail[hold_i][1]), float(trail[hold_i][2])
        d_hold = math.hypot(hx - sx, hy - sy)

        gen = int(getattr(leader, "_follow_trail_gen", 0) or 0)
        if getattr(self, "_follow_trail_seen_gen", None) != gen:
            self._follow_trail_seen_gen = gen
            self._follow_next_id = int(trail[hold_i][0])

        # 오래된 점이 잘려도 트레일 맨 앞을 되밟지 않음 — 홀드로 붙음
        first_sid = int(trail[0][0])
        if int(getattr(self, "_follow_next_id", 0) or 0) < first_sid:
            self._follow_next_id = int(trail[hold_i][0])

        i = _index_for_id(self._follow_next_id)
        if i is None:
            return
        if i > hold_i:
            i = hold_i
        ...
        path = getattr(self, "path", None) or []
        if path:
            px0, py0 = float(path[0][0]), float(path[0][1])
            for k in range(i, hold_i + 1):
                if math.hypot(float(trail[k][1]) - px0, float(trail[k][2]) - py0) <= 1.5:
                    i = k
                    self._follow_next_id = int(trail[k][0])
                    break

        # 홀드가 더 가까우면 앞 트레일을 포기 (리더 근처에서 맵 전체를 되걷지 않음)
        d_i = math.hypot(float(trail[i][1]) - sx, float(trail[i][2]) - sy)
        if d_hold + 8.0 < d_i:
            i = hold_i
            self._follow_next_id = int(trail[hold_i][0])
            d_i = d_hold
        ...
        # 홀드까지 직선이 멀면 바로 붙기
        if d_hold > reconnect_px:
            _reconnect_to(hx, hy, force=True)
            return
        if getattr(self, "_path_plan_job", None) is not None:
            self._path_plan_job = None

        while i < hold_i and math.hypot(float(trail[i][1]) - sx, float(trail[i][2]) - sy) <= on_pt:
            self._follow_next_id = int(trail[i][0]) + 1
            i += 1

        # 옛 점을 너무 많이 넣지 않음 — 홀드 직전 look_n 점만 등속 보행
        if hold_i - i > look_n:
            i = hold_i - look_n
            self._follow_next_id = int(trail[i][0])

        if i >= hold_i and d_hold <= on_pt:
            if path:
                self.path = []
                self.state = "idle"
            return

        _set_trail_polyline(_pts_from(i, hold_i))
    ...
    def stop_moving(self, preserve_anim_override=False):
        if not preserve_anim_override:
            self.clear_anim_override()
        # 점프 중이면 지면으로 복구한 뒤 path/target 정리 (공중 좌표 고정 방지)
        self._clear_jump_arc(restore_pos=True)
        self.path = []
        self.target = list(self.pos)
        if not preserve_anim_override:
            self.state = "idle"
        self._hop_repeat = None  # ACTION_ANIM jump loop 반복 hop 해제
        self.event_waypoints = None
    ...
    def start_hop(
        ...
        *,
        ...
        """
        연출용 점프 — jump 애니 + _jump_arc 포물선(발 그림자·공중 높이).
        land=None 이면 제자리. repeat=True 이면 착지 후 같은 스펙으로 다시 뜀
        (ACTION_ANIM jump mode:hold + loop:true).
        반환: 이번 hop duration_ms (대기 시간 계산용).
        """
        try:
            x = float(self.pos[0])
            y = float(self.pos[1])
        ...
        try:
            if arc_h is None:
                ah = float(CONFIG.get("JUMP_ARC_HEIGHT", 50) or 50)
            else:
                ah = float(arc_h)
        except (TypeError, ValueError):
            ah = float(CONFIG.get("JUMP_ARC_HEIGHT", 50) or 50)
        ah = max(4.0, float(_clamp_draw_height(ah) or ah))
        # 착지 — land 미지정이면 제자리 (연출 점프). 짧은 이동 착지는 호출측에서 land 전달.
        ex, ey = x, y
        if land is not None and isinstance(land, (list, tuple)) and len(land) >= 2:
            try:
                ex, ey = float(land[0]), float(land[1])
            except (TypeError, ValueError):
                ex, ey = x, y
        dist = math.hypot(ex - x, ey - y)
        if dur_ms is None:
            if dist < 0.5:
                # 제자리 hop: 도랑 점프용 MIN(~220ms)은 너무 짧음 → 연출 전용 기본 시간
                try:
                    base = float(CONFIG.get("JUMP_HOP_DURATION_MS", 750) or 750)
                except Exception:
                    base = 750.0
                try:
                    ref_h = float(CONFIG.get("JUMP_ARC_HEIGHT", 50) or 50)
                except Exception:
                    ref_h = 50.0
                scale = max(0.75, min(2.5, float(ah) / max(1.0, ref_h)))
                dur_ms = int(base * scale)
                try:
                    dmin = int(CONFIG.get("JUMP_MIN_DURATION_MS", 220) or 220)
                except Exception:
                    dmin = 220
                try:
                    dmax = int(CONFIG.get("JUMP_HOP_DURATION_MAX_MS", 1800) or 1800)
                except Exception:
                    dmax = 1800
                dur_ms = int(max(dmin, min(dmax, dur_ms)))
            else:
                _ah, dur = _compute_jump_params(
                    0.0, getattr(self, "jump_max_gap", 30) or 30, max(dist, 12.0), jump_height_px=ah
                )
                dur_ms = int(dur)
        else:
            try:
                dur_ms = int(max(80, float(dur_ms)))
            except (TypeError, ValueError):
                dur_ms = 400
        now = pygame.time.get_ticks()
        self.path = [(ex, ey, 1)]
        ...
        self.last_anim_time = 0
        try:
            _event_refresh_facing_image(self)
        except Exception:
            pass
        if repeat:
            self._hop_repeat = {
                "arc_h": float(ah),
                "dur_ms": int(dur_ms),
                "inplace": land is None,
            }
        else:
            self._hop_repeat = None
        return int(dur_ms)

    def _restart_hop_if_repeating(self):
        """착지 직후 — ACTION_ANIM jump loop 이면 같은 스펙으로 재점프."""
        spec = getattr(self, "_hop_repeat", None)
        if not isinstance(spec, dict):
            return False
        try:
            x = float(self.pos[0])
            ...
            return False
        ah = float(spec.get("arc_h") or CONFIG.get("JUMP_ARC_HEIGHT", 50) or 50)
        dur_ms = int(spec.get("dur_ms") or 400)
        now = pygame.time.get_ticks()
        self.path = [(x, y, 1)]
        self.target = [x, y]
        self._jump_arc = {
...
def blit_field_wall_sprite(
    ...
    *,
    ...
    """
    맵 XY 임의 각도 밑변 + 세로 픽셀은 화면 위(빌보드와 호환)인 스프라이트 벽.
    wall_angle 0°=+X, 90°=+Y. current_img 는 원본(또는 FX 적용) 비트맵.
    force=True(wall_3d) 이면 0°여도 컬럼 렌더.

    3D_ROTATE(Mode7): 열마다 월드 점을 투영한다.
      · 높이 = 그 점의 깊이 스케일 (가까운 쪽 크고 먼 쪽 작음)
      · 가로 = 인접 열 화면 X 간격 (1px 고정이면 가까운 쪽에 빈 줄이 생김)
    eff_z 는 카메라 줌×엔티티 줌만. Mode7 깊이는 여기에 넣지 않는다.
    """
    if screen is None or current_img is None:
        return False
    if not field_wall_angle_active(wall_angle_deg, force=bool(force)):
        return False
    try:
        ez = float(eff_z) if eff_z is not None else float(zoom)
    except (TypeError, ValueError):
        ez = float(zoom)
    if ez <= 1e-6:
        return False
    ...
    if a < 255:
        try:
            ...
            pass
    try:
        rw = int(render_img.get_width())
        rh = int(render_img.get_height())
    except Exception:
        return False
    if rw < 1 or rh < 1:
        return False
    try:
        w0 = float(native_w) if native_w is not None else float(current_img.get_width())
    except Exception:
        w0 = float(rw) / max(1e-6, float(ez))
    w0 = max(1.0, w0)
    half = 0.5 * w0
    ang = _clamp_wall_angle_deg(wall_angle_deg)
    c, s = field_wall_dir_xy(ang)
    z = float(zoom)
    h_world = float(height or 0.0)
    try:
        vw = float(view_w if view_w is not None else CONFIG.get("WIDTH", 640))
    except Exception:
        vw = 640.0
    try:
        screen_w = int(CONFIG.get("WIDTH", 640))
        screen_h = int(CONFIG.get("HEIGHT", 480))
    except Exception:
    ...
    ends = []
    for sign in (-1.0, 1.0):
        wx = float(world_x) + sign * half * c
        wy = float(world_y) + sign * half * s
        if mode7_ctx:
            pr = rotate3d_mode7_project(
                wx, wy, mode7_ctx, height_off=h_world, zoom=z
            )
            if pr and pr.get("valid", pr.get("visible")):
                ends.append((float(pr["sx"]), float(pr["sy"]), float(pr.get("scale", 1.0) or 1.0)))
        else:
            sx, sy = _field_world_to_screen_anchor(
                wx,
                wy,
                cam_x,
                cam_y,
            ...
            ends.append((float(sx), float(sy), 1.0))
    if not ends:
        return False
    min_sx = min(e[0] for e in ends) - 2.0
    max_sx = max(e[0] for e in ends) + 2.0
    min_sy = min(e[1] - rh * max(0.05, e[2]) for e in ends) - 2.0
    max_sy = max(e[1] for e in ends) + 2.0
    if max_sx < -8 or min_sx > screen_w + 8 or max_sy < -8 or min_sy > screen_h + 8:
        return False

    # 열 투영 → 인접 열 화면 간격만큼 가로를 늘린다.
    # i 가 끊기면(카메라 뒤 등) 그 구간은 늘리지 않는다.
    samples = []
    for i in range(rw):
        along = -half + (float(i) + 0.5) * (w0 / float(rw))
        ...
        if mode7_ctx:
            ...
            try:
                sc = max(0.05, float(pr.get("scale", 1.0) or 1.0))
            except (TypeError, ValueError):
                sc = 1.0
            col_h = max(1, int(round(float(rh) * sc)))
        else:
            sx, sy = _field_world_to_screen_anchor(
                wx,
                wy,
        ...
        samples.append((i, float(sx), float(sy), int(col_h)))

    blit = screen.blit
    Rect = pygame.Rect
    n_s = len(samples)
    max_col_w = max(1, int(screen_w))
    for k in range(n_s):
        i, sx, sy, col_h = samples[k]
        bx = int(round(sx))
        col_w = 1
        if k + 1 < n_s and samples[k + 1][0] == i + 1:
            x0 = bx
            x1 = int(round(samples[k + 1][1]))
            if x1 == x0:
                col_w = 1
            else:
                bx = min(x0, x1)
                col_w = min(max_col_w, abs(x1 - x0))
        by = int(round(sy)) - int(col_h)
        try:
            if col_w == 1 and col_h == rh:
                blit(render_img, (bx, by), area=Rect(i, 0, 1, rh))
            else:
                col = render_img.subsurface(Rect(i, 0, 1, rh))
                if col_w != 1 or col_h != rh:
                    col = pygame.transform.scale(col, (int(col_w), int(col_h)))
                blit(col, (bx, by))
        except Exception:
            pass
    return True
...
class FieldItem:
    def __init__(
        ...
        if wall_angle is not None:
            self.wall_angle = _clamp_wall_angle_deg(wall_angle)
        else:
            self.wall_angle = _clamp_wall_angle_deg(info.get("wall_angle", 0.0))
        if wall_3d is not None:
            self.wall_3d = field_wall_3d_flag(wall_3d)
        else:
            self.wall_3d = field_wall_3d_flag(info.get("wall_3d", False))
    ...
    def draw(self, screen, cam_x, cam_y, player=None, global_frame=0, zoom=1.0, y_transform=None, x_offset_fn=None, sprite_perspective_q=None, shear_lod=False, x_scale_fn=None, x_shift_fn=None, pivot_xy=None, cam_angle_rad=0.0, mode7_ctx=None, view_w=None):
        ...
        wall_ang = _clamp_wall_angle_deg(getattr(self, "wall_angle", 0.0))
        wall_draw = field_item_is_wall(self)
        if mode7_ctx:
            ...
            if wall_draw and not self.is_slot:
                # 벽: 중심이 카메라 뒤여도 양 끝은 보일 수 있음. 깊이는 blit 열마다.
                if m7 and m7.get("valid", m7.get("visible")):
                    dx_base, dy_base = float(m7["sx"]), float(m7["sy"])
                else:
                    dx_base, dy_base = -1e6, -1e6
            else:
        ...
        # --- [3. 최적화 조건문 (이미지 크기 반영)] ---
        # 벽: blit_field_wall_sprite 내부에서 양끝 투영으로 컬링.
        if not wall_draw:
        ...
        # 벽 각도: 밑변을 맵 XY 각도로 세우고 세로 픽셀은 화면 위
        if wall_draw and not self.is_slot:
            blit_field_wall_sprite(
                screen,
                current_img,
                float(self.pos[0]),
                float(self.pos[1]),
                ...
                eff_z=eff_z,
                sprite_tilt=getattr(self, "sprite_tilt", 1.0),
                sprite_perspective_q=None if mode7_ctx else sprite_perspective_q,
                y_transform=y_transform,
                x_offset_fn=x_offset_fn,
                x_scale_fn=x_scale_fn,
                x_shift_fn=x_shift_fn,
                ...
                mode7_ctx=mode7_ctx,
                view_w=view_w,
                native_w=current_img.get_width(),
                force=field_wall_3d_flag(getattr(self, "wall_3d", False)),
            )
        ...
        try:
            item_a = int(getattr(self, "alpha", 255))
        except (TypeError, ValueError):
            item_a = 255
        prepared = _prepare_field_sprite_blit(
            current_img,
...
def _parse_optional_step_bool(raw):
    """SAY show_name 등 3상태. None/빈칸=기본값, JSON false 는 False 유지 (bool 과 int 혼동 주의)."""
    if raw is None:
        return None
...
def _resolve_action_anim_frame_sec(step, anim_name: str):
    ...
    if isinstance(raw, (list, tuple)):
        ...
        try:
            ...
            return None
    try:
        secs = [max(0.01, float(x)) for x in items]
    except (TypeError, ValueError):
        return None
    return secs if secs else None


def action_anim_persist_spec_from_step(step) -> dict | None:
    """ACTION_ANIM persist 세이브용 스펙. anim 없으면 None."""
    if not isinstance(step, dict):
        return None
    anim = (step.get("anim") or step.get("name") or step.get("state") or "").strip()
    if not anim:
        return None
    mode = (step.get("mode") or "hold").strip().lower() or "hold"
    if mode not in ("once", "hold"):
        mode = "hold"
    release = (step.get("release") or "idle").strip().lower() or "idle"
    if release not in ("idle", "stop"):
        release = "idle"
    spec = {"anim": anim, "mode": mode, "release": release}
    if "loop" in step:
        raw = step.get("loop")
        if isinstance(raw, str):
            spec["loop"] = raw.strip().lower() not in ("0", "false", "f", "no", "n", "off", "")
        else:
            spec["loop"] = bool(raw)
    ...
    if d in ("left", "l"):
        ...
        spec["dir"] = "right"
    if "height" in step and step.get("height") is not None and str(step.get("height")).strip() != "":
        try:
            spec["height"] = float(step.get("height"))
        except (TypeError, ValueError):
            pass
    return spec


def apply_persisted_action_anim(ent, spec, *, mask=None, objs=None, npcs=None) -> bool:
    """
    placed/세이브 action_anim 또는 ACTION_ANIM persist 스펙을 엔티티에 적용.
    jump+hold 는 가능하면 start_hop(repeat), 아니면 play_anim hold.
    """
    if ent is None or not isinstance(spec, dict):
        return False
    anim = (spec.get("anim") or spec.get("name") or spec.get("state") or "").strip()
    if not anim:
        return False
    mode = (spec.get("mode") or "hold").strip().lower() or "hold"
    if mode not in ("once", "hold"):
        mode = "hold"
    release = (spec.get("release") or "idle").strip().lower() or "idle"
    if release not in ("idle", "stop"):
        release = "idle"
    anim_l = anim.lower()
    if "loop" in spec:
        ...
        loop = False
    d = (spec.get("dir") or "").strip().lower()
    if d in ("left", "l") and hasattr(ent, "direction"):
        ent.direction = "left"
        try:
            _event_refresh_facing_image(ent)
        except Exception:
            pass
    elif d in ("right", "r") and hasattr(ent, "direction"):
        ent.direction = "right"
        try:
            _event_refresh_facing_image(ent)
        except Exception:
    ...
    hop_h = None
    if anim_l == "jump":
        if "height" in spec and spec.get("height") is not None:
            try:
                hop_h = float(spec.get("height"))
            except (TypeError, ValueError):
                hop_h = None
        if hop_h is None:
            try:
                hop_h = float(CONFIG.get("JUMP_ARC_HEIGHT", 50) or 50)
            except Exception:
                hop_h = 50.0
    ...
    if (
        anim_l == "jump"
        and mode == "hold"
        and hasattr(ent, "start_hop")
        and callable(getattr(ent, "start_hop", None))
    ):
        try:
            ent.start_hop(
                arc_h=hop_h,
            ...
            print(f"[action_anim persist hop] {e}")
            used_hop = False
    if not used_hop:
        pa = getattr(ent, "play_anim", None)
        if callable(pa):
            pa(
                anim,
                duration_ms=0 if mode == "hold" else max(50, int(float(spec.get("val") or 1.0) * 1000)),
                loop=loop if mode == "hold" else False,
                release=release,
                temp_height=hop_h if anim_l == "jump" else None,
            )
        elif hasattr(ent, "state"):
            ent.state = anim_l
    ...
    if d in ("left", "right"):
        clean["dir"] = d
    if hop_h is not None and anim_l == "jump":
        clean["height"] = float(hop_h)
    try:
        ent._action_anim_persist = True
        ent._persisted_action_anim = clean
    except Exception:
        pass
    return True


def clear_persisted_action_anim(ent) -> None:
    """ACTION_ANIM persist 해제 + hop/오버라이드 정리."""
    if ent is None:
        return
    try:
        ent._action_anim_persist = False
        ent._persisted_action_anim = None
    except Exception:
        pass
    try:
        if getattr(ent, "_hop_repeat", None):
            ent._hop_repeat = None
    except Exception:
        pass
...
def _anim_once_pos_from_step(step):
    """ANIM_ONCE 첫 좌표 (하위 호환)."""
    pts = _anim_once_positions_from_step(step)
    return pts[0] if pts else (0.0, 0.0)


def _anim_once_positions_from_step(step):
    """
    ANIM_ONCE pos:
    - [x, y] → 한 곳
    - [[x, y], [x, y], ...] → 같은 애니를 여러 곳에 동시 재생
    """
    raw = step.get("pos", [0, 0])
...
def _entity_matches_event_target(ent, tn: str) -> bool:
    ...
    if iid and iid == t:
        return True
    try:
        if str(getattr(ent, "name", "") or "") == t:
            return True
    except Exception:
        pass
    # CHANGE persist 없이 외형만 바뀐 경우 — placed 세이브 키로도 찾는다
    try:
        save_nm = str(getattr(ent, "_placed_save_name", "") or "").strip()
    except Exception:
        save_nm = ""
    return bool(save_nm) and save_nm == t
...
_ALL_NPCS_TOKENS = frozenset({"all_npcs", "allnpc", "all_npc", "*npcs"})


def _is_all_npcs_token(tok) -> bool:
    return str(tok or "").strip().lower() in _ALL_NPCS_TOKENS


def _split_event_target_tokens(raw) -> list:
    """
    target 필드 → 토큰 목록.
    'a,b' / 'a; b' / ['a','b'] / all_npcs 지원.
    """
    if raw is None:
        return []
    if isinstance(raw, (list, tuple)):
        out = []
        for item in raw:
            out.extend(_split_event_target_tokens(item))
        return out
    s = str(raw).strip()
    if not s:
        return []
    # 쉼표·세미콜론 구분 (공백만으로는 나누지 않음)
    buf = []
    cur = []
    for ch in s:
        if ch in ",;":
...
def _event_group_spacing_px(step=None) -> float:
    """다중 target 단체 배치 간격/원형 반경. step.spacing 우선, 없으면 CONFIG."""
    if isinstance(step, dict) and step.get("spacing") is not None and str(step.get("spacing")).strip() != "":
        try:
            return max(0.0, float(step.get("spacing")))
        except (TypeError, ValueError):
            pass
    try:
        return max(0.0, float(CONFIG.get("EVENT_GROUP_SPACING_PX", 50) or 50))
    except (TypeError, ValueError):
        return 50.0


def _event_group_layout_mode(step=None) -> str:
    """
    단체 배치 방식.
    circle — 지정 좌표를 중심으로 원형(둘러싸기)
    left / right / up / down — 앵커에서 해당 방향으로 일렬
    """
    raw = ""
    ...
    if m in aliases:
        return aliases[m]
    try:
        default = str(CONFIG.get("EVENT_GROUP_LAYOUT", "circle") or "circle").strip().lower()
    except Exception:
        default = "circle"
    return aliases.get(default, "circle")


def _event_group_offset_xy(index: int, count: int, spacing: float = 50.0, layout: str = "circle"):
    """
    단체 배치 오프셋 (앵커=지정 좌표 기준).
    - circle: 전원 원주 위 (spacing=반경). 1명이면 앵커.
    - right/left/up/down: 0번 앵커, 이후 해당 방향으로 spacing 씩.
    """
    import math
    ...
    if sp < 0:
        sp = 0.0
    lay = str(layout or "circle").strip().lower() or "circle"

    if n <= 1:
        return 0.0, 0.0

    if lay == "circle":
        # 위쪽(-Y)부터 시계 방향(또는 수학각)으로 균등 배치
        ang = -math.pi / 2.0 + (2.0 * math.pi * float(i) / float(n))
        return math.cos(ang) * sp, math.sin(ang) * sp
    if lay == "right":
        return float(i) * sp, 0.0
    if lay == "left":
        return -float(i) * sp, 0.0
    if lay == "up":
        return 0.0, -float(i) * sp
    if lay == "down":
        return 0.0, float(i) * sp
    # 알 수 없으면 원형
    ang = -math.pi / 2.0 + (2.0 * math.pi * float(i) / float(n))
...
def _event_group_pos(base_x, base_y, index: int, count: int, spacing: float = 50.0, layout: str = "circle"):
    ox, oy = _event_group_offset_xy(index, count, spacing, layout=layout)
    return float(base_x) + ox, float(base_y) + oy


def _event_offset_waypoints(wps, index: int, count: int, spacing: float = 50.0, layout: str = "circle"):
    """웨이포인트 전체에 동일 단체 오프셋 적용."""
    if not wps:
        return []
    ox, oy = _event_group_offset_xy(index, count, spacing, layout=layout)
    ...
    for p in wps:
        try:
            out.append([float(p[0]) + ox, float(p[1]) + oy])
        except (TypeError, ValueError, IndexError):
            continue
    return out
...
class Camera:
    ...
    def update(self, player, npcs, objs, map_w, map_h, shear_screen_px=0.0, dt_sec=1.0 / 60.0):
        ...
        # - 기본: 카메라 줌=1, 확대/축소는 후처리(world_zoom_draw)로 한 장을 스케일.
        # - _force_view_zoom 이 있으면(논리 320 고정+줌아웃) 카메라 시야만 넓혀
        #   320 버퍼를 꽉 채운다. 후처리 축소는 검은 테두리가 생긴다.
        fz = getattr(self, "_force_view_zoom", None)
        if fz is not None:
            try:
                z_use = max(1e-6, float(fz))
            except (TypeError, ValueError):
                z_use = 1.0
            self.current_zoom = z_use
            self.target_zoom = z_use
        else:
            self.current_zoom = 1.0
            self.target_zoom = 1.0
...
def _effective_outline_px(font: pygame.font.Font, outline_px: int) -> int:
    """요청 테두리 두께를 글자 높이에 맞게 줄임.
    예전엔 h<=12 에서 0 으로 꺼 버려서 SELECTBOX 안내(12px 흰 글자)가 필드 위에서 안 보였음.
    작은 글자도 1px 은 유지. 8px 이하만 글리프가 잠기므로 생략.
    """
    try:
        px = int(outline_px)
    except Exception:
...
def _event_apply_step_dir(target, step, player=None) -> bool:
    """
    MOVE/PLACE 등: dir(face) 적용. 성공 시 True.
    - left / right
    - player (= face_player): 플레이어 쪽을 바라봄. 대상이 player 이면 변화 없음(no-op).
    방향이 바뀌면 현재 스프라이트(image)도 즉시 갱신 — ANIM_DELAY 를 기다리지 않음.
    """
    if target is None:
        return False
    d = (step.get("dir") or step.get("face") or "").strip().lower()
    if d in ("left", "l"):
        ...
        _event_refresh_facing_image(target)
        return True
    if d in ("right", "r"):
        target.direction = "right"
        _event_refresh_facing_image(target)
        return True
    if d in ("player", "face_player", "toward_player", "to_player", "look_player"):
        if player is not None and (target is player):
            return True  # 대상이 player + dir:player → 무시
        if player is None or not hasattr(target, "direction"):
            return False
        try:
            target.direction = (
                "left"
                if float(player.pos[0]) < float(target.pos[0])
                else "right"
            )
        except Exception:
            return False
...
def _event_refresh_facing_image(target) -> None:
    """
    direction 변경 직후 self.image 를 해당 방향 프레임으로 맞춤.
    PLACE 직후 update_anim ANIM_DELAY 전에 그리면 left 프레임이 남는 문제 방지.
    """
    if target is None or not hasattr(target, "direction"):
        return
    try:
        d = str(getattr(target, "direction", "left") or "left").strip().lower()
        anims = getattr(target, "anims_r" if d == "right" else "anims_l", None)
        if not isinstance(anims, dict):
            return
        st = str(getattr(target, "state", "idle") or "idle")
        ao = getattr(target, "_anim_override", None)
        if isinstance(ao, dict) and ao.get("state"):
            st = str(ao.get("state") or st)
        frames = anims.get(st) or anims.get("idle")
        if not frames:
            return
        idx = int(getattr(target, "frame_idx", 0) or 0)
        if idx < 0:
            idx = 0
        idx = idx % len(frames)
        target.image = frames[idx]
    except Exception:
        pass
...
class EventManager:
    def __init__(self, flow_ref, music_mgr=None):
        ...
        self.is_notice = False  # NOTICE: 중앙 안내창 (SAY 대화창과 별도)
        self.is_busy = False    
        self.wait_timer = 0
        self.current_who = ""
        self.current_text = ""
        # 세이브 가드용: 최근 프레임의 맵·플레이어 (start_event 체크포인트)
        self._runtime_map_id = None
        self._runtime_player = None
        ...
        # NOTICE 자동 닫기: auto=true 이면 타자 완료 후 val초 뒤 페이드아웃
        self._notice_auto = False
        self._notice_auto_val = 0.0
        self._notice_title = ""
        ...
        self._change_fade = None  # CHANGE speed/fade — 크로스페이드 진행 dict (완료 시 next_step)
        ...
        # SCREEN 오버레이는 출력 모드를 바꾸지 않음 (이미지는 현재 논리 화면에 스케일)
        ...
        self.game_options_button_visible = False  # GAME_OPTIONS_BUTTON 으로 표시
        self._game_options_button_visible_snapshot = False
        self._game_options_button_persist = False
        self.cursor_visible = True # 커서 가시성 제어 추가
        self._cursor_visible_persist = False  # True면 이번 이벤트 CURSOR_VISIBLE 을 종료 후에도 유지
        self._cursor_visible_snapshot = True  # 이벤트 시작 직전 커서 표시 상태
        self._event_music_touched = False  # 이번 이벤트에서 MUSIC_PLAY/STOP/END 를 씀
        self._event_music_persist = False  # True면 종료 후에도 그 BGM(또는 무음) 유지
        self._map_default_music = ""
        self._map_default_music_loop = True
        self._map_default_music_volume = None
    ...
    def start_event(
        ...
        *,
        ...
        self._game_debug_button_persist = False
        self._game_options_button_visible_snapshot = bool(getattr(self, "game_options_button_visible", False))
        self._game_options_button_persist = False
        # 커서: 시작 시점 상태를 기억. persist 없는 CURSOR_VISIBLE 은 종료 시 여기로 복구.
        # (예전처럼 종료 때마다 True 로 강제하면, 숨긴 뒤 다른 이벤트만 끝나도 커서가 다시 뜸)
        self._cursor_visible_snapshot = bool(getattr(self, "cursor_visible", True))
        self._cursor_visible_persist = False
        self._event_music_touched = False
        self._event_music_persist = False
        # 이전 이벤트의 FADE color(흰색 등)가 다음 이벤트에 남지 않게
        ...
        self._event_if_taken = False  # CONDITION if/elif 체인: 앞에서 맞은 분기 있음
        ...
        # 중도 종료 대비: 이벤트 시작 직전 맵·좌표·진행도를 디스크에 고정
        try:
            if self.flow and hasattr(self.flow, "capture_pre_event_checkpoint"):
                mid = map_id if map_id is not None else getattr(self, "_runtime_map_id", None)
                pos = player_pos
                if pos is None:
                    pl = getattr(self, "_runtime_player", None)
                    if pl is not None:
                        pos = getattr(pl, "pos", None)
                self.flow.capture_pre_event_checkpoint(mid, pos)
        except Exception:
            pass
    ...
    def _apply_fade_color_from_step(self, step):
        """FADEIN/FADEOUT color/rgb → fade_color.
        비어 있으면 이번 이벤트 안에서 직전에 쓴 색 유지(같은 이벤트 FADEOUT→FADEIN 짝).
        새 이벤트 시작 시 fade_color 는 검정으로 리셋됨.
        """
        if not isinstance(step, dict):
            return
        raw = step.get("color")
    ...
    def _apply_overlay_ui_step(self, step):
        ...
        # text/button: {progress_status} 등 세이브·세션 변수 치환 (SAY 와 동일 expand_story_text)
        step_use = step
        ct_pre = (step.get("content") or step.get("content_type") or "text").strip().lower()
        if ct_pre in ("text", "button", ""):
            raw_txt = step.get("text")
            if raw_txt is None:
                raw_txt = step.get("label")
            expanded = self._expand_overlay_ui_text(raw_txt)
            if str(expanded) != str(raw_txt or ""):
                step_use = dict(step)
                step_use["text"] = expanded
            # 순수 텍스트이고 치환 결과가 비면 show 하지 않음 (boot 복원 시 빈 진행 문구)
            if ct_pre in ("text", "") and not str(expanded or "").strip():
                return

        surf = _build_overlay_surface_from_step(step_use)
        if surf is None:
            return
        sw = int(CONFIG.get("WIDTH", 320))
        sh = int(CONFIG.get("HEIGHT", 240))
        w, h = surf.get_size()
        anchor = step_use.get("anchor") or "center"
        mx = _overlay_ui_px(float(step_use.get("margin_x") or 0), screen_w=sw, step=step_use)
        my = _overlay_ui_px(float(step_use.get("margin_y") or 0), screen_w=sw, step=step_use)
        rx, ry = _overlay_anchor_xy(anchor, w, h, mx, my, sw, sh)
        mode = (step_use.get("mode") or "fade").strip().lower()
        if mode not in ("fade", "scroll"):
            mode = "fade"
        scroll_enter = (step_use.get("scroll_enter") or step_use.get("from") or "left").strip().lower()
        try:
            appear = float(step_use.get("appear") if step_use.get("appear") is not None else step_use.get("appear_sec", 0.5))
        except Exception:
            appear = 0.5
        try:
            disappear = float(step_use.get("disappear") if step_use.get("disappear") is not None else step_use.get("disappear_sec", 0.5))
        except Exception:
            disappear = 0.5
        hold_forever = bool(step_use.get("hold_forever") or step_use.get("infinite_hold"))
        hold_raw = step_use.get("hold") if "hold" in step_use else step_use.get("hold_sec")
        if hold_forever:
            t_hold_ms = None
        else:
            try:
        ...
        oid = (step_use.get("overlay_id") or step_use.get("id") or "").strip()
        if not oid:
            oid = "ov_" + uuid.uuid4().hex[:10]
        persist = bool(step_use.get("persist", False))

        sx, sy = _scroll_enter_delta(scroll_enter, w, h, sw, sh)
        ex, ey = _scroll_exit_delta(scroll_enter, w, h, sw, sh)

        ct = (step_use.get("content") or step_use.get("content_type") or "text").strip().lower()
        now = pygame.time.get_ticks()
        ov = {
            "id": oid,
            "surf": surf,
            ...
            "draw_dy": 0.0,
            "clickable": bool(
                step_use.get("clickable") or step_use.get("button") or ct == "button"
            ),
            "click_action": (step_use.get("click_action") or step_use.get("on_click") or "").strip(),
        }
        if mode == "scroll":
            ov["draw_dx"] = float(sx)
            ov["draw_dy"] = float(sy)
    ...
    def _expand_overlay_ui_text(self, text):
        """OVERLAY_UI text — SAY 와 같은 템플릿 + 세션 변수 덮어쓰기."""
        try:
            from data import expand_story_text
            from flow import eval_session_vars

            sd = dict(self._story_save_data())
            try:
                sp = eval_session_vars(getattr(self, "flow", None))
                if isinstance(sp, dict) and sp:
                    sd.update(sp)
            except Exception:
                pass
            return expand_story_text(text, sd)
        except Exception:
            return str(text or "")
    ...
    def _story_save_data(self):
        flow = getattr(self, "flow", None)
        sd = getattr(flow, "save_data", None) if flow is not None else None
        return sd if isinstance(sd, dict) else {}

    def _sync_place_persist(self, step, targets, player, *, is_remove=False) -> None:
        """
        PLACE 스텝 → save_data['placed'] 동기화.
        - action:remove → 이름별로 placed 에서 삭제 (persist 여부 무관 — 정리)
        - persist:true → upsert (맵·좌표·dir·behavior·travel)
        - persist 없음 → 세이브에 쓰지 않음 (런타임 1회성)
        인트로/데모(boot_phase<2)에서도 메모리 upsert 는 허용(새게임 꽃 등).
        디스크 기록은 main 온보딩 세이브 가드가 막는다.
        """
        from field_runtime import parse_step_bool, parse_step_persist
        from flow import (
            clear_entity_placed_persist,
            mark_entity_placed_persist,
            remove_placed_entity,
            upsert_placed_entity,
        )

        sd = self._story_save_data()
        mid = str(sd.get("current_map") or "")
        want_persist = parse_step_persist(step, default=False)
        travel_raw = step.get("travel", None)
        travel = None
        if travel_raw is not None:
            travel = parse_step_bool(travel_raw, False)

        for target in targets or []:
            if target is None or target is player:
                continue
            nm = str(getattr(target, "name", "") or "").strip()
            if not nm:
                continue
            if is_remove:
                save_nm = str(getattr(target, "_placed_save_name", "") or "").strip()
                remove_placed_entity(sd, nm)
                if save_nm and save_nm != nm:
                    remove_placed_entity(sd, save_nm)
                clear_entity_placed_persist(target)
                continue
            if not want_persist:
                continue
            beh = step.get("behavior") or step.get("behavior_mode") or step.get("mode")
            dir_s = step.get("dir") or step.get("face")
            if isinstance(dir_s, str) and dir_s.strip().lower() in (
                "player",
                "face_player",
                "toward_player",
                "to_player",
                ...
                dir_s = getattr(target, "direction", None)
            upsert_placed_entity(
                sd,
                name=nm,
                map_id=mid,
                pos=getattr(target, "pos", None),
                dir=dir_s if isinstance(dir_s, str) else getattr(target, "direction", None),
                behavior=beh,
                travel=travel,
                sprite_tilt=step.get("sprite_tilt")
                if "sprite_tilt" in step
                else getattr(target, "sprite_tilt", None),
                height=step.get("height") if "height" in step else None,
                ysort=step.get("ysort") if "ysort" in step else None,
                layer=step.get("layer") if "layer" in step else None,
                radius=step.get("radius"),
                interval_ms=step.get("interval_ms"),
            )
            mark_entity_placed_persist(target, nm)

    def _sync_behavior_to_placed(self, target, mode, step) -> None:
        """BEHAVIOR 스텝 — 이미 placed 인 NPC 의 behavior/follow 를 세이브에 반영."""
        if target is None:
            return
        nm = str(getattr(target, "name", "") or "").strip()
        if not nm:
            return
        try:
            from flow import find_placed_entry, update_placed_behavior
            from field_runtime import parse_step_bool, parse_step_persist

            sd = self._story_save_data()
            # persist:true 인 BEHAVIOR 는 placed 에 없으면 upsert 까지
            # (PLACE persist 없이 follow 만 영속화하고 싶을 때)
            entry = find_placed_entry(sd, nm)
            want_upsert = parse_step_persist(step, default=False)
            travel_raw = step.get("travel", None)
            travel = parse_step_bool(travel_raw, None) if travel_raw is not None else None
            if entry is None and want_upsert:
                ...
                upsert_placed_entity(
                    ...
                    map_id=mid,
                    pos=getattr(target, "pos", None),
                    dir=getattr(target, "direction", None),
                    behavior=mode,
                    travel=travel,
                    radius=step.get("radius"),
                    interval_ms=step.get("interval_ms"),
                )
                from flow import mark_entity_placed_persist
            ...
            print(f"[BEHAVIOR placed] {e}")

    def _resolve_event_target_name(self, name):
        """player_parent / allyN / allyN_parent / {kid}_parent 등 → 실제 char/obj id. player 는 그대로."""
        try:
            from data import resolve_story_target_id

            return resolve_story_target_id(name, self._story_save_data())
        except Exception:
        ...
        for tok in tokens:
            if _is_all_npcs_token(tok):
                for n in npcs or []:
                    ...
                    if eid in seen:
                        continue
                    if player is not None and n is player:
                        continue
                    seen.add(eid)
                    out.append(n)
                continue
            name = self._resolve_event_target_name(tok)
            if not name:
                continue
            nl = name.strip().lower()
            if nl == "player":
                if allow_player and player is not None:
                    eid = id(player)
                    if eid not in seen:
                        seen.add(eid)
                        out.append(player)
    ...
    def _expand_story_say_text(self, text):
        try:
            from data import expand_story_text

            expanded = expand_story_text(text, self._story_save_data())
        except Exception:
            expanded = str(text or "")
        # JSON/에디터 한 줄의 \\n → 실제 개행 (타자·줄바꿈 공통)
        s = str(expanded or "").replace("\r\n", "\n").replace("\r", "\n")
        if "\\" in s:
            s = s.replace("\\n", "\n").replace("\\r", "")
        return s
    ...
    def _resolve_story_who_label(self, who):
        ...
        try:
            ...
            return resolve_story_who_label(who, self._story_save_data())
        except Exception:
            return str(who or "")
    ...
    def _resolve_head_entity(self, name, head_ctx):
        if not head_ctx:
            return None
        n = self._resolve_event_target_name(name)
    ...
    def _say_ui_fade_alpha_for_overlays(self):
        """SAY 텍스트박스 / NOTICE 패널과 동일 페이드 알파(대화·안내 중이 아니면 255)."""
        if not (
            bool(getattr(self, "is_talking", False))
            or bool(getattr(self, "is_notice", False))
        ):
            return 255
        try:
    ...
    def _configure_say_bubble_from_step(self, step):
        ...
        tokens = _split_event_target_tokens(tgt_raw)
        if not tokens:
            return
        names = []
        seen = set()
        for tok in tokens:
            if _is_all_npcs_token(tok):
                # 이름은 나중에 draw 시 head_ctx npcs 로 확장 — 토큰 유지
                key = "all_npcs"
                if key not in seen:
                    seen.add(key)
                    names.append(key)
                continue
            try:
                nm = self._resolve_event_target_name(tok)
            except Exception:
                nm = str(tok or "").strip()
        ...
        self._say_bubbles = bubbles
        # 하위 호환: 단일 참조
        self._say_bubble = bubbles[0] if bubbles else None

    def _iter_say_bubbles(self):
        bbs = getattr(self, "_say_bubbles", None)
    ...
    def _tick_say_bubble_frame(self, dt_sec=1.0 / 60.0):
        ...
        if dt_ms <= 0:
            return
        for bb in self._iter_say_bubbles():
            if not bb or not bb.get("frames"):
                continue
    ...
    def _say_bubble_target_names(self, bb, head_ctx):
        """말풍선 1개 항목 → 실제 부착 대상 이름 목록 (all_npcs 확장)."""
        nm = (bb.get("target_name") if isinstance(bb, dict) else "") or ""
        if _is_all_npcs_token(nm):
            out = []
            for x in (head_ctx or {}).get("npcs") or []:
                n = getattr(x, "name", "") or ""
                if n:
                    out.append(n)
            return out
        ...
        if isinstance(raw, (list, tuple)) and raw:
            ...
            for tok in raw:
                ...
                if nm and nm not in seen:
                    ...
                    names.append(nm)
            return names
        tgt = (em.get("target") or "").strip()
        if _is_all_npcs_token(tgt):
            return [
                getattr(x, "name", "")
                for x in ((head_ctx or {}).get("npcs") or [])
                if getattr(x, "name", "")
            ]
        return [tgt] if tgt else ["player"]
    ...
    def _draw_head_attached_ui(self, screen: pygame.Surface, head_ctx):
        ...
        if em and em.get("frames"):
            ...
            frame = em["frames"][fi]
            for tname in self._emote_overlay_target_names(em, head_ctx):
                ent = self._resolve_head_entity(tname, head_ctx)
                anc = _head_top_center_screen(ent, head_ctx)
                if not anc:
                    continue
                cx, top_y = map_head_screen(*anc)
                blit_icon(frame, cx, top_y, ox, oy, 255)

        for bb in self._iter_say_bubbles():
            if not bool(getattr(self, "is_talking", False)) or not bb or not bb.get("frames"):
                continue
            try:
            ...
            for tname in self._say_bubble_target_names(bb, head_ctx):
                ent = self._resolve_head_entity(tname, head_ctx)
                anc = _head_top_center_screen(ent, head_ctx)
                if not anc:
                    continue
                cx, top_y = map_head_screen(*anc)
                blit_icon(frame, cx, top_y, ox, oy, al)
    ...
    def draw_ui_overlays(self, screen: pygame.Surface, head_ctx=None, *, layer="all"):
        ...
        if draw_dialog:
            try:
                ...
                pass
        # NOTICE: 화면 어둡게 + 중앙 안내 패널 (SAY 대화창과 별도)
        if draw_dialog and bool(getattr(self, "is_notice", False)):
            try:
                self._draw_notice_panel(screen)
            except Exception:
                pass
    ...
    def _tick_say_typewriter(self):
        ...
        if not full:
            ...
            self._say_done = True
            try:
                now_empty = int(pygame.time.get_ticks())
            except Exception:
                now_empty = 0
            try:
            ...
            if bool(getattr(self, "is_notice", False)):
                self._arm_dialog_close_timer(now_empty, hold_ms)
            else:
                self._say_can_close_at_ms = 0
            return
        try:
        ...
        if nxt >= len(full):
            ...
            try:
                ...
                hold = 0.5
            hold_ms = int(max(0.0, min(3.0, hold)) * 1000.0)
            self._arm_dialog_close_timer(now, hold_ms)

    def _arm_dialog_close_timer(self, now_ms, hold_ms):
        """타자 완료 후 닫기 가능 시각. NOTICE auto 면 val초와 최소 대기 중 큰 값."""
        close_ms = int(now_ms) + int(max(0, hold_ms))
        if bool(getattr(self, "is_notice", False)) and bool(getattr(self, "_notice_auto", False)):
            try:
                extra = float(getattr(self, "_notice_auto_val", 0.0) or 0.0)
            except Exception:
                extra = 0.0
            extra_ms = int(max(0.0, min(120.0, extra)) * 1000.0)
            close_ms = int(now_ms) + max(int(hold_ms), extra_ms)
        self._say_can_close_at_ms = close_ms
    ...
    def _apply_say_step(self, step, *, chain=False):
        ...
        try:
            expanded = self._expand_story_say_text(raw_text)
        except Exception:
            expanded = str(raw_text or "")
        self.current_text = expanded
        try:
            self._say_full_text = str(expanded or "")
        except Exception:
            self._say_full_text = ""
        self._say_show_name = _parse_optional_step_bool(step.get("show_name", None))
        self._say_visible_n = 0
        self._say_last_char_ms = 0
    ...
    def _apply_notice_step(self, step):
        """NOTICE: 화면을 어둡게 하고 중앙 패널에 설명글. SAY 타자·페이드를 재사용."""
        self.is_notice = True
        self.is_talking = False
        self._say_bubble = None
        self._say_bubbles = []
        ...
        self.current_who = ""
        try:
            self._notice_title = str(step.get("name") or step.get("title") or "").strip()
        except Exception:
            self._notice_title = ""
        raw_text = step.get("text", "")
        try:
        ...
        self._say_done = False
        self._say_can_close_at_ms = 0
        self._notice_auto = self._parse_step_bool(step.get("auto", False), default=False)
        try:
            self._notice_auto_val = float(step.get("val") if step.get("val") is not None else 0.0)
        except (TypeError, ValueError):
            self._notice_auto_val = 0.0
        self._notice_auto_val = max(0.0, min(120.0, float(self._notice_auto_val)))
        try:
            hold_open = float(CONFIG.get("SAY_MIN_OPEN_DELAY_SEC", 0.5) or 0.5)
        except Exception:
            hold_open = 0.5
    ...
    def _draw_notice_panel(self, screen):
        """NOTICE 딤 + 중앙 패널 + 타자 텍스트 (논리 해상도 서피스)."""
        try:
            ...
            try:
                ...
                w, h = 320, 240
        say_alpha = int(self._say_ui_fade_alpha_for_overlays())
        say_alpha = max(0, min(255, say_alpha))
        if say_alpha <= 0:
            return
        try:
            dim0 = int(CONFIG.get("NOTICE_DIM_ALPHA", 180) or 180)
        except Exception:
            dim0 = 180
        dim0 = max(0, min(255, dim0))
        dim_a = int(round(dim0 * (say_alpha / 255.0)))
        if dim_a > 0:
            dim = pygame.Surface((w, h), pygame.SRCALPHA)
            dim.fill((0, 0, 0, dim_a))
            screen.blit(dim, (0, 0))

        def _cfg_px(key, default):
            try:
                v = float(CONFIG.get(key, default) or default)
            except Exception:
                v = float(default)
            return int(round(_scale_px_from_320(v, screen_w=w)))

        panel_w = max(80, min(w - 16, _cfg_px("NOTICE_PANEL_W_320", 248)))
        pad = max(6, _cfg_px("NOTICE_PANEL_PAD_320", 12))
        max_h = max(48, min(h - 16, _cfg_px("NOTICE_PANEL_MAX_H_320", 180)))
        line_gap = max(1, _cfg_px("NOTICE_LINE_GAP_PX_320", 3))
        try:
            fs0 = float(CONFIG.get("NOTICE_FONT_SIZE_320", 11) or 11)
        except Exception:
            fs0 = 11.0
        try:
            nfs0 = float(CONFIG.get("NOTICE_TITLE_SIZE_320", 13) or 13)
        except Exception:
            nfs0 = 13.0
        fs = max(6, int(round(_scale_px_from_320(fs0, screen_w=w))))
        nfs = max(6, int(round(_scale_px_from_320(nfs0, screen_w=w))))
        ...
        fkey = str(body_p.get("font_key") or CONFIG.get("SAY_FONT_KEY", "dialog") or "dialog").strip() or "dialog"
        font = _resolve_ui_font(fkey, fs, outlined=False)
        title_font = _resolve_ui_font(fkey, nfs, outlined=False)
        text_col = _normalize_rgb_color(CONFIG.get("NOTICE_TEXT_COLOR", (235, 240, 250)))
        title_col = _normalize_rgb_color(CONFIG.get("NOTICE_TITLE_COLOR", (255, 248, 220)))
        bg = _normalize_rgb_color(CONFIG.get("NOTICE_PANEL_BG", (22, 30, 48)))
        border = _normalize_rgb_color(CONFIG.get("NOTICE_PANEL_BORDER", (150, 175, 210)))
        try:
            ol_on = bool(body_p.get("outline_enabled", CONFIG.get("SAY_FONT_OUTLINE_ENABLED", True)))
        except Exception:
            ol_on = True
        ...
        try:
            ...
            nvis = len(full)
        vis = full[: max(0, min(len(full), nvis))]
        inner_w = max(8, panel_w - pad * 2)
        title = str(getattr(self, "_notice_title", "") or "").strip()
        title_h = 0
        if title:
            try:
                _tw, title_h = title_font.size(title)
            except Exception:
                title_h = nfs + 2
            title_h = int(title_h) + line_gap + 2
        lines = _wrap_text_by_pixels(vis, font, inner_w)
        body_h = 0
        line_hs = []
        for ln in lines:
            try:
                _lw, lh = font.size(ln if ln else " ")
            except Exception:
                lh = fs + 2
            line_hs.append(int(lh))
            body_h += int(lh) + line_gap
        if line_hs:
            body_h -= line_gap
        panel_h = min(max_h, max(36, title_h + body_h + pad * 2))
        px = (w - panel_w) // 2
        py = (h - panel_h) // 2
        panel = pygame.Surface((panel_w, panel_h), pygame.SRCALPHA)
        br = max(4, int(round(_scale_px_from_320(6, screen_w=w))))
        pygame.draw.rect(panel, (*bg, 235), (0, 0, panel_w, panel_h), border_radius=br)
        pygame.draw.rect(panel, (*border, 255), (0, 0, panel_w, panel_h), max(1, int(round(_scale_px_from_320(1, screen_w=w)))), border_radius=br)
        if say_alpha < 255:
            panel.set_alpha(say_alpha)
        screen.blit(panel, (px, py))
        clip_prev = screen.get_clip()
        screen.set_clip(pygame.Rect(px + pad, py + pad, inner_w, max(1, panel_h - pad * 2)))
        cur_y = py + pad
        if title:
            try:
                if ol_px > 0:
                    _blit_text_with_outline(
                        screen,
                        title_font,
                        title,
                        px + pad,
                    ...
                    ts = title_font.render(title, True, title_col)
                    if say_alpha < 255:
                        ts = ts.copy()
                        ts.set_alpha(say_alpha)
                    screen.blit(ts, (px + pad, cur_y))
            except Exception:
                pass
            cur_y += title_h
        for i, ln in enumerate(lines):
            if cur_y >= py + panel_h - pad:
                break
            try:
                if ol_px > 0:
                    _blit_text_with_outline(
                    ...
                    if say_alpha < 255:
                        s_ln = s_ln.copy()
                        s_ln.set_alpha(say_alpha)
                    screen.blit(s_ln, (px + pad, cur_y))
            except Exception:
                break
            lh = line_hs[i] if i < len(line_hs) else fs
            cur_y += int(lh) + line_gap
    ...
    def _say_finish_after_fade_out(self):
        """SAY/NOTICE 페이드아웃 완료 후 정리하고 다음 스텝으로."""
        self._say_ui_fade_phase = None
        was_notice = bool(getattr(self, "is_notice", False))
        if was_notice:
            self.is_notice = False
            self._notice_auto = False
            self._notice_title = ""
    ...
    def advance_dialog(self):
        ...
        if em and em.get("awaiting_click"):
            ...
            self.next_step()
            return
        if bool(getattr(self, "is_talking", False)) or bool(getattr(self, "is_notice", False)):
            try:
                full = str(self._say_full_text or self.current_text or "")
            except Exception:
                full = ""
            ...
            # 연속 SAY: 박스 닫지 않고 다음 대사만 적용 (NOTICE 는 체인 없음)
            if not bool(getattr(self, "is_notice", False)):
                if self._say_peek_next_is_say() and self._try_advance_say_chain():
                    return

            self._begin_dialog_ui_fade_out(now)
            return

        if self.try_advance_screen():
            return

    def _begin_dialog_ui_fade_out(self, now_ms=0):
        """SAY/NOTICE 창 닫기 페이드아웃 시작."""
        try:
            fade_en = bool(CONFIG.get("SAY_UI_FADE_ENABLED", True))
        except Exception:
            fade_en = True
        ...
        if fade_en and fout_ms > 0:
            ...
            self._say_ui_fade_t0_ms = int(now_ms) if now_ms else 0
            self._say_ui_fade_elapsed = 0.0
        else:
            self._say_finish_after_fade_out()

    def _tick_notice_auto(self):
        """NOTICE auto=true: 타자 완료+val 후 자동 닫기."""
        if not bool(getattr(self, "is_notice", False)):
            return
        if not bool(getattr(self, "_notice_auto", False)):
            return
        if getattr(self, "_say_ui_fade_phase", None) == "out":
            return
        if not bool(getattr(self, "_say_done", False)):
            return
        try:
            now = int(pygame.time.get_ticks())
        except Exception:
    ...
    def _follow_formation_goal(self, lead, dist, slot, slots):
        """
        (레거시) 리더 facing 기준 대형 슬롯 좌표.
        현재 동행은 트레일 위 slot*FOLLOW_MULTI_SPACING 만큼 뒤로 밀어 같은 길을 따라간다.
        """
        try:
    ...
    def _tick_event_followers(self, player, npcs, objs):
        ...
        리더 발자국을 한 번만 샘플하고, 팔로워는 그 트레일을 따라간다 (A* 없음).
        기본 속도는 리더 sync 속도 × FOLLOW speed.
        hold 거리보다 벌어지면 FOLLOW_CATCHUP_* 로 잠깐 가속해 간격을 좁힌다.
        """
        ...
        try:
            ...
            spacing = 18.0
        try:
            catch_start = float(CONFIG.get("FOLLOW_CATCHUP_START_PX", 10) or 10)
        except Exception:
            catch_start = 10.0
        try:
            catch_sync = float(CONFIG.get("FOLLOW_CATCHUP_SYNC_PX", 4) or 4)
        except Exception:
            catch_sync = 4.0
        try:
            catch_mul = float(CONFIG.get("FOLLOW_CATCHUP_SPEED_MUL", 1.35) or 1.35)
        except Exception:
            catch_mul = 1.35
        catch_start = max(0.0, catch_start)
        catch_sync = max(0.0, min(catch_start, catch_sync))
        catch_mul = max(1.0, catch_mul)
        ...
        for f in list(self._followers):
            ...
            if kid in sampled:
                continue
            sampled.add(kid)
            sampler = getattr(lead, "follow_trail_sample", None)
            if callable(sampler):
                try:
                    sampler()
                except Exception:
        ...
        # 체인(A→B→C): 리더가 팔로워가 아닌 쪽부터 속도를 맞춰야 한 프레임에 전파됨
        fol_ids = set()
        for f in list(self._followers):
            e = ent.get(f.get("follower"))
            if e is not None:
                fol_ids.add(id(e))

        def _follow_tick_rank(row):
            lead_e = ent.get((row or {}).get("leader"))
            if lead_e is not None and id(lead_e) in fol_ids:
                return 1
            return 0

        for f in sorted(list(self._followers), key=_follow_tick_rank):
            ...
            try:
                ...
                dist = 40.0
            try:
                rel = float(f.get("speed", 1.0) or 1.0)
            except Exception:
                rel = 1.0
            # 리더의 catch-up 가속은 빼고 sync 기준만 물려받음 (체인 배율 폭주 방지)
            try:
                lead_sync = getattr(lead, "_follow_sync_speed_mul", None)
                if lead_sync is None:
                    lead_sync = getattr(lead, "event_speed_mul", 1.0)
                lead_mul = float(lead_sync or 1.0)
            except Exception:
                lead_mul = 1.0
            base = max(0.0, lead_mul * rel)
            try:
                fol._follow_sync_speed_mul = base
            except Exception:
                pass
            ...
            try:
                ...
                slots_n = 1
            if slots_n > 1 and f.get("slot") is not None:
                try:
                    hold_dist = dist + float(f.get("slot", 0) or 0) * spacing
                except Exception:
                    hold_dist = dist

            try:
                d_lead = math.hypot(
                    float(lead.pos[0]) - float(fol.pos[0]),
                    float(lead.pos[1]) - float(fol.pos[1]),
                )
            except Exception:
                d_lead = hold_dist
            slack = d_lead - hold_dist
            catching = bool(getattr(fol, "_follow_catching_up", False))
            if slack > catch_start:
                catching = True
            elif slack <= catch_sync:
                catching = False
            try:
                fol._follow_catching_up = catching
            except Exception:
                pass
            spm = base * catch_mul if catching else base

            if isinstance(fol, MaskWalkingCharacter):
                try:
                    fol.follow_step(lead.pos, hold_dist, m, objs, npcs, speed_mul=spm, leader=lead)
                except Exception:
                    pass
            else:
                try:
    ...
    def update(self, player, camera, objs, npcs, mask_img=None, dt_sec=1.0 / 60.0, map_id=None):
        ...
        try:
            ...
            pass
        self._runtime_npcs = npcs
        self._runtime_objs = objs
        ...
        if self._escape_mode == "condition" and self._escape_condition:
            ...
            try:
                ...
                ctx.update(eval_session_vars(self.flow))
            except Exception:
        ...
        if self.step_idx < len(self.active_event):
            ...
            while self.step_idx < len(self.active_event) and burst < 24:
                ...
                if st == "CONDITION":
                    # if: 매번 평가 (연쇄). elif/else: 앞에서 맞았으면 이 블록은 SKIP 까지 건너뜀.
                    chain = condition_step_chain_mode(step)
                    taken = bool(getattr(self, "_event_if_taken", False))
                    if chain in ("elif", "else") and taken:
                        skip_depth += 1
                    else:
                        if chain == "else":
                            ok = True
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    pass
                            ok = evaluate_event_step_condition(step, ctx)
                        if not ok:
                            skip_depth += 1
                        if chain == "if":
                            self._event_if_taken = bool(ok)
                        elif ok:
                            self._event_if_taken = True
                    self._event_skip_depth = skip_depth
                    self.next_step()
                    burst += 1
                    continue
    ...
    def _run_char_anim_step(self, step, player, npcs, objs):
        """ANIM(레거시) + ACTION_ANIM(모드·방향·점프 높이·release). 다중 target 지원."""
        s_type = (step.get("type") or "ANIM").upper()
        targets = self._resolve_event_targets(
            step.get("target") or "player",
            player,
            npcs,
            objs,
            default_to_player=True,
        )
        anim_name = (step.get("name") or step.get("anim") or step.get("state") or "").strip()
        if not anim_name or not targets:
            self.next_step()
            return
        ...
        d = (step.get("dir") or step.get("face") or "").strip().lower()
        # jump + height: MaskWalkingCharacter 는 실제 _jump_arc 포물선,
        # 그 외(BaseCharacter) 는 기존처럼 temp_height 고정 띄우기.
        hop_h = None
        if anim_name == "jump" and ("height" in step or str(step.get("height", "")).strip() != ""):
            try:
                hop_h = float(step.get("height"))
            except (TypeError, ValueError):
                hop_h = None
        elif anim_name == "jump":
            try:
        ...
        hop_repeat = bool(mode == "hold" and loop and anim_name == "jump")
        max_hop_ms = 0
        mask_img = getattr(self, "_event_mask_img", None)
        # jump hop 1회 시간: once 는 val/duration, hold 도 val>0 이면 사용(미지정 시 start_hop 자동)
        hop_dur_ms = None
        if anim_name == "jump":
            if mode == "once" and duration_ms > 0:
                hop_dur_ms = int(duration_ms)
            else:
                try:
                    v_hop = float(step.get("val", 0) or 0)
                except (TypeError, ValueError):
                    v_hop = 0.0
                if v_hop > 0.05:
                    hop_dur_ms = int(max(80, v_hop * 1000.0))
        ...
        for target in targets:
            ...
            # MOVE 기본 wait:false 이면 아직 path 가 남은 채 ACTION_ANIM 이 시작될 수 있다.
            # 도착 시 stop_moving() 이 anim/hop 을 지워 idle 로 돌아가므로, 애니 적용 전에 이동만 끊는다.
            try:
                sm = getattr(target, "stop_moving", None)
                if callable(sm):
                ...
                try:
                    target.path = []
                    target.target = list(target.pos)
                except Exception:
                    pass

            used_hop = False
            if anim_name == "jump" and hasattr(target, "start_hop") and callable(getattr(target, "start_hop", None)):
                try:
                    # once: 제자리 포물선 1회 / hold+loop: 착지마다 반복
                    ms = target.start_hop(
                        arc_h=hop_h,
                        dur_ms=hop_dur_ms,
                        land=None,  # 제자리 (연출용). 이동 점프는 MOVE/도랑 경로 사용
                        repeat=hop_repeat,
                        mask=mask_img,
                        objs=objs,
                        npcs=npcs,
                    ...
                    used_hop = True
                    if ms:
                        max_hop_ms = max(max_hop_ms, int(ms))
                except Exception as e:
                    print(f"[ACTION_ANIM jump hop] {e}")
                    used_hop = False
        ...
        # ACTION_ANIM persist:true → 이벤트 종료·세이브(placed)에도 유지
        try:
            from field_runtime import parse_step_persist

            if "persist" in step or "keep_after_event" in step or "hold_after_event" in step:
                want_persist = bool(parse_step_persist(step, default=False))
                if want_persist:
                    spec = action_anim_persist_spec_from_step(step)
                    if spec:
                        for target in targets:
                            if target is player:
                                continue
                            try:
                                target._action_anim_persist = True
                                target._persisted_action_anim = dict(spec)
                            except Exception:
                                pass
                            self._sync_action_anim_to_placed(target, spec)
                else:
                    for target in targets:
                        if target is player:
                            continue
                        clear_persisted_action_anim(target)
                        self._sync_action_anim_to_placed(target, None)
        except Exception as e:
            print(f"[ACTION_ANIM persist] {e}")

        # once+wait: 점프 arc 시간과 val 중 더 긴 쪽까지 대기
        wait_ms = duration_ms
        if anim_name == "jump" and max_hop_ms > 0:
            wait_ms = max(wait_ms, max_hop_ms)
        if wait_ms > 0 and wait_for_finish and mode == "once":
            self._anim_wait_end_ms = pygame.time.get_ticks() + int(wait_ms)
        else:
            self._anim_wait_end_ms = 0
            self.next_step()

    def _sync_action_anim_to_placed(self, target, spec) -> None:
        """ACTION_ANIM persist → save_data['placed'][].action_anim 동기화."""
        if target is None:
            return
        nm = str(getattr(target, "name", "") or "").strip()
        if not nm or nm.lower() == "player":
            return
        try:
            from flow import find_placed_entry, upsert_placed_entity

            sd = self._story_save_data()
            entry = find_placed_entry(sd, nm)
            if entry is None:
                if not bool(getattr(target, "_placed_persist", False)):
                    return
                # PLACE persist 된 대상이면 entry 보장
                mid = str(sd.get("current_map") or "")
                upsert_placed_entity(
                    sd,
                    name=nm,
            ...
            if entry is None:
                return
            if isinstance(spec, dict) and (spec.get("anim") or spec.get("name")):
                entry["action_anim"] = dict(spec)
            else:
                entry.pop("action_anim", None)
        except Exception as e:
            print(f"[placed action_anim] {e}")
    ...
    def _event_move_start_on_target(self, fragment, target, player, npcs, objs, wait_for_finish):
        ...
        if not wps:
            # pos 없이 dir 만: 제자리 방향 전환 (예: { "type":"MOVE", "target":"c10", "dir":"player" })
            if _event_apply_step_dir(target, fragment, player=player):
        ...
        if instant:
            ...
            if move_anim:
                ...
                if callable(pa):
                    pa(move_anim, duration_ms=0, loop=True, release="idle", temp_height=None)
            if "dir" in fragment:
                _event_apply_step_dir(target, fragment, player=player)
            return "instant"
    ...
    def _spawn_anim_once(self, step):
        """
        ANIM_ONCE: object_defs 키(name) + 월드 pos — 애니 1회 재생.
        pos 가 [[x,y], ...] 이면 같은 프레임에 여러 곳. 기본은 전부 끝날 때까지 대기.
        wait:false 면 즉시 다음 스텝 (연속 ANIM_ONCE 는 한 프레임에 같이 시작).

        JSON 필드:
          name, pos — 필수 (pos 는 [x,y] 또는 [[x,y],[x,y],...])
          frame_ms / anim_delay_ms — 프레임 간격(ms, 비우면 object_defs 기본)
          speed / anim_speed — 재생 배율 (2=2배 빠름). frame_ms와 함께 쓰면 배율 적용
          wait — 재생 끝까지 대기 (기본 true)
        """
        e_name = (step.get("name") or "").strip()
        if not e_name:
            return False
        positions = _anim_once_positions_from_step(step)
        delay = _effect_anim_delay_from_step(step, obj_name=e_name)
        spawned = []
        for px, py in positions:
            new_effect = Effect(
                e_name, px, py, loop=False, anchor="feet", anim_delay_ms=delay
            )
            self.active_effects.append(new_effect)
            spawned.append(new_effect)
        if not spawned:
            return False
        wait_done = True
        if step.get("wait") is not None:
            wait_done = _parse_effect_step_wait(step)
        if wait_done:
            self._effect_wait_refs = list(spawned)
            self._effect_wait_ref = spawned[-1]
        else:
            self._effect_wait_refs = None
    ...
    def _execute_change_step(self, step, player, npcs, objs):
        """
        [CHANGE] FieldItem/캐릭터 외형 교체. target 다중·all_npcs 지원.
        held/@held 는 단일만.
        speed(초)=변신 시간(옛·새 스프라이트 겹침). fade 도 같은 크로스페이드.
        speed 가 있으면 speed, 없으면 fade. 둘 다 0이면 즉시. 다중 대상은 즉시.
        persist:true 이면 save_data.placed 의 name 을 to 키로 바꿔 재시작 후에도 외형을 유지.
        object_defs ↔ char_defs 교차 교체도 가능 (시든 꽃 오브젝트 → 요정 캐릭터).
        """
        from field_runtime import parse_step_persist

        tgt_raw = step.get("target") or step.get("from") or ""
        new_name = (step.get("to") or step.get("new_name") or step.get("name") or "").strip()
        if not new_name:
            print("[CHANGE] to(새 object_defs / char_defs 키) 필요")
            self.next_step()
            return
        want_persist = parse_step_persist(step, default=False)
        ...
        tokens = _split_event_target_tokens(tgt_raw)
        items = []
        held_tokens = {"held", "@held", "player.held", "hand"}
        if len(tokens) == 1 and str(tokens[0]).strip().lower() in held_tokens:
            ...
            want_held = False
            for tok in tokens:
                if str(tok).strip().lower() in held_tokens:
                    want_held = True
                else:
                    name_tokens.append(tok)
            if want_held:
                held = getattr(player, "held_item", None)
                if held is not None:
                    items.append(held)
            if name_tokens:
                for ent in self._resolve_event_targets(name_tokens, player, npcs, objs):
                    if isinstance(ent, (FieldItem, BaseCharacter)) and ent not in items:
                        items.append(ent)
        else:
            item = getattr(player, "held_item", None)
            if item is None:
        ...
        if not items:
            print(f"[CHANGE] 대상 없음: {tgt_raw!r}")
            self.next_step()
            return
        ...
        try:
            ...
            fade_sec = 0.0
        try:
            speed_sec = float(step.get("speed", step.get("speed_sec", 0)) or 0)
        except (TypeError, ValueError):
            speed_sec = 0.0
        morph_sec = speed_sec if speed_sec > 0.0 else fade_sec

        # 다중 대상은 즉시 교체 (speed/fade 는 단일만)
        if morph_sec > 0.0 and len(items) == 1 and isinstance(items[0], (BaseCharacter, FieldItem)):
            item = items[0]
            try:
                start_a = int(getattr(item, "alpha", 255) or 255)
            except (TypeError, ValueError):
                start_a = 255
            start_a = max(0, min(255, start_a))
            if start_a <= 0:
                start_a = 255
            ghost = snapshot_change_ghost(item)
            item = self._apply_change_retarget(
                item, new_name, persist=want_persist, player=player, npcs=npcs, objs=objs
            )
            if item is None:
                self.next_step()
                return
            ...
            if ghost:
                ghost["alpha"] = start_a
                item._change_ghost = ghost
            else:
                item._change_ghost = None
            self._change_fade = {
                "item": item,
                "new_name": new_name,
                "applied": True,
                "persist": want_persist,
                "sec": max(0.05, float(morph_sec)),
                "elapsed": 0.0,
                "start_alpha": start_a,
            }
            self.is_busy = True
        ...
        for item in items:
            _clear_entity_change_ghost(item)
            self._apply_change_retarget(
                item, new_name, persist=want_persist, player=player, npcs=npcs, objs=objs
            )
        self.next_step()

    def _change_def_kind(self, name: str):
        """CHANGE to 키가 오브젝트인지 캐릭터인지. 둘 다 아니면 None."""
        key = str(name or "").strip()
        if not key:
            return None
        if key in OBJ_ASSETS:
            return "obj"
        if key in CHAR_ASSETS:
            return "char"
        return None

    def _spawn_entity_for_change(self, name, pos, src):
        """CHANGE 교차 교체용 스폰. PLACE 와 같은 FieldItem / BaseCharacter 경로."""
        try:
            pos_xy = [float(pos[0]), float(pos[1])]
        except (TypeError, ValueError, IndexError):
            pos_xy = [0.0, 0.0]
        st = getattr(src, "sprite_tilt", None)
        h = getattr(src, "height", None)
        ys = getattr(src, "ysort_mode", None)
        ly = getattr(src, "layer", None)
        if name in OBJ_ASSETS:
            return FieldItem(
                name,
                pos_xy[0],
                pos_xy[1],
                sprite_tilt=st if st is not None else 1.0,
                height=h,
                ysort_mode=ys,
                layer=ly,
            )
        ...
        entry = {"name": name, "pos": list(pos_xy)[:2]}
        if spawn_as_mask_walker(name, entry):
            ent = MaskWalkingCharacter(name, pos_xy, ch_info)
        else:
            ent = BaseCharacter(name, pos_xy, ch_info)
        try:
            attach_npc_from_entry(ent, entry)
        except Exception:
            pass
        return ent

    def _swap_change_entity_in_scene(self, old_ent, new_ent, npcs, objs, player=None):
        """objs/npcs/_active_entities 에서 old 를 빼고 new 를 넣는다."""
        if old_ent is None or new_ent is None or old_ent is new_ent:
            return
        if player is not None and getattr(player, "held_item", None) is old_ent:
            try:
                player.held_item = None
            except Exception:
                pass
        for lst in (objs, npcs):
            if lst is None:
                continue
            try:
                while old_ent in lst:
                    lst.remove(old_ent)
            except Exception:
                pass
        dest = objs if isinstance(new_ent, FieldItem) else npcs
        if dest is not None and new_ent not in dest:
            dest.append(new_ent)
        ae = getattr(self, "_active_entities", None)
        if isinstance(ae, list):
            replaced = False
            for i, x in enumerate(ae):
                if x is old_ent:
                    ae[i] = new_ent
                    replaced = True
            if not replaced:
                ae.append(new_ent)
        fols = getattr(self, "_followers", None)
        if isinstance(fols, list):
            for f in fols:
                if not isinstance(f, dict):
                    continue
                if f.get("ent") is old_ent:
                    f["ent"] = new_ent
                if str(f.get("follower") or "") == str(getattr(old_ent, "name", "") or ""):
                    f["follower"] = str(getattr(new_ent, "name", "") or "")

    def _sync_change_persist(self, item, old_name, new_name) -> None:
        """CHANGE persist:true — placed 항목 이름을 to 키로 맞추고 좌표를 갱신한다."""
        from flow import (
            mark_entity_placed_persist,
            rename_placed_entity,
            upsert_placed_entity,
        )
        ...
        entry = rename_placed_entity(sd, old_nm, new_nm)
        upsert_placed_entity(
            sd,
            name=new_nm,
            map_id=mid,
            pos=getattr(item, "pos", None),
            dir=getattr(item, "direction", None),
            sprite_tilt=getattr(item, "sprite_tilt", None),
            height=getattr(item, "height", None),
            ysort=getattr(item, "ysort_mode", None),
            layer=getattr(item, "layer", None),
        )
        if not entry:
            print(f"[CHANGE persist] placed 신규: {old_nm or '?'} -> {new_nm}")
        else:
            print(f"[CHANGE persist] placed {old_nm or entry.get('name')} -> {new_nm}")
        mark_entity_placed_persist(item, new_nm)

    def _apply_change_retarget(
        self, item, new_name, *, persist=False, player=None, npcs=None, objs=None
    ):
        """CHANGE 외형 교체. 같은 종류면 retarget, 오브젝트↔캐릭터면 엔티티를 교체.

        성공 시 현재 엔티티(교체됐으면 새 인스턴스)를 반환. 실패 시 None.
        """
        if item is None:
            return None
        old = getattr(item, "name", "?")
        new_nm = str(new_name or "").strip()
        new_kind = self._change_def_kind(new_nm)
        if new_kind is None:
            print(f"[CHANGE] 실패: to='{new_nm}' 이 object_defs/char_defs 에 없음")
            return None
        old_kind = "char" if isinstance(item, BaseCharacter) else (
            "obj" if isinstance(item, FieldItem) else None
        )
        if old_kind is None:
        ...
        if old_kind == new_kind:
            if old_kind == "char":
                ok = bool(item.retarget_char_def(new_nm))
                tag = "char"
            else:
                ok = bool(item.retarget_object_def(new_nm))
                tag = "obj"
            if not ok:
                print(f"[CHANGE] 실패: to='{new_nm}' 없음({tag})")
                return None
            print(f"[CHANGE] {old} -> {new_nm} ({tag})")
        else:
            if player is not None and getattr(player, "held_item", None) is item and new_kind == "char":
                print("[CHANGE] 들고 있는 물건을 캐릭터로 바꿀 수 없음")
                return None
            new_ent = self._spawn_entity_for_change(new_nm, getattr(item, "pos", [0, 0]), item)
            if new_ent is None:
                print(f"[CHANGE] 실패: {old} -> {new_nm} 스폰")
                return None
            d = str(getattr(item, "direction", "") or "").strip().lower()
            if d in ("left", "right") and hasattr(new_ent, "direction"):
                new_ent.direction = d
                try:
                    _event_refresh_facing_image(new_ent)
                except Exception:
                    pass
            save_key = str(getattr(item, "_placed_save_name", "") or "").strip()
            was_placed = bool(getattr(item, "_placed_persist", False))
            self._swap_change_entity_in_scene(item, new_ent, npcs, objs, player=player)
            live = new_ent
            print(f"[CHANGE] {old} -> {new_nm} ({old_kind}→{new_kind})")
            if was_placed and not persist:
                from flow import mark_entity_placed_persist

                mark_entity_placed_persist(live, save_key or old)

        if persist:
            try:
                self._sync_change_persist(live, old, new_nm)
            except Exception as e:
                print(f"[CHANGE persist] {e}")
        return live
    ...
    def _execute_step(self, step, player, camera, npcs, objs):
        ...
        if s_type == "SAY":
            ...
            # 화면을 어둡게 하고 중앙 안내창. 클릭/A 또는 auto+val 로 닫기.
            self._apply_notice_step(step)
        elif s_type == "EMOTE":
            ...
            if isinstance(par, (list, tuple)) and len(par) > 0:
                ...
                for frag in par:
                    if not isinstance(frag, dict):
                        continue
                    tn = self._resolve_event_target_name(frag.get("target"))
            ...
            if move_sync:
                ...
                if len(bundle_idx) >= 2:
                    ...
                    for bi in bundle_idx:
                        st = ev[bi]
                        tn = self._resolve_event_target_name(st.get("target"))
            ...
            move_tn = step.get("target")
            targets = self._resolve_event_targets(move_tn, player, npcs, objs)
            if not targets:
                self.next_step()
            else:
                ...
                if isinstance(w, str):
                    ...
                    wait_for_finish = bool(w)
                spacing = _event_group_spacing_px(step)
                layout = _event_group_layout_mode(step)
                n_tg = len(targets)
                ...
                for i, target in enumerate(targets):
                    ...
                    if wps and n_tg > 1:
                        frag = dict(step)
                        ow = _event_offset_waypoints(wps, i, n_tg, spacing, layout=layout)
                        if len(ow) == 1:
                            frag["pos"] = ow[0]
                        else:
                            frag["pos"] = ow
                    res = self._event_move_start_on_target(
                        frag, target, player, npcs, objs, wait_for_finish
                    )
                    if res == "skip":
                        continue
                    any_applied = True
                ...
                if not any_applied:
                    self.next_step()
                elif started_moving and wait_for_finish and not instant:
                    pass  # busy 유지 — _check_completion 에서 전원 도착 대기
                else:
                    self.next_step()
            ...
            # reverse_tilt 명시 시에만 기록 → 생략이면 맵 field.reverse_tilt 유지
            if pt.get("reverse_tilt") is not None:
                tc["reverse_tilt"] = bool(pt["reverse_tilt"])
            ...
            if mode in ("follow_entity", "follow", "entity"):
                cmd["target"] = self._resolve_event_target_name(pcam.get("target") or "")
            ...
            from field_runtime import parse_result_step, result_step_is_session
            ...
            if patch:
                self._apply_event_result_to_save(
                    ...
                    source="step",
                    session=result_step_is_session(step if isinstance(step, dict) else {}),
                )
            ...
            self._event_call_stack.append(
                {
                    ...
                    "skip_depth": int(getattr(self, "_event_skip_depth", 0) or 0),
                    "if_taken": bool(getattr(self, "_event_if_taken", False)),
                }
            )
            self._fragment_call_depth += 1
            self._fragment_call_set.add(call_id)
            self._event_skip_depth = 0
            self._event_if_taken = False
            ...
            # 컷신용 트레일 follow. persist:true 이면 ambient follow + save_data.placed 에도 기록
                ...
                for follower in add_names:
                    ...
                    if fol_ent is not None:
                        try:
                            ...
                            set_npc_behavior(fol_ent, "follow")
                            from flow import mark_entity_placed_persist

                            mark_entity_placed_persist(fol_ent)
                        except Exception:
                            pass
            ...
            if fol_tokens:
                ...
                self._followers = []
            try:
                used_leads = {str(x.get("leader") or "").strip() for x in self._followers}
                for e in [player] + list(npcs or []) + list(objs or []):
                    if e is None:
                        continue
                    nm = str(getattr(e, "name", "") or "").strip()
                    iid = str(getattr(e, "instance_id", "") or "").strip()
                    if (nm and nm in used_leads) or (iid and iid in used_leads):
                        continue
                    if getattr(e, "_follow_trail", None):
                        clr = getattr(e, "follow_trail_clear", None)
                        if callable(clr):
                            clr()
            except Exception:
                pass
            ...
            v = step.get("val", True)
            is_visible = parse_step_bool(v, True)
            self.cursor_visible = bool(is_visible)
            # persist:true → 이벤트 종료 후에도 유지. 없으면 종료 시 시작 직전 스냅샷으로 복구.
            if "persist" in step or "keep_after_event" in step or "hold_after_event" in step:
                self._cursor_visible_persist = bool(parse_step_persist(step, default=False))
            else:
                # persist 키 생략 = 이 이벤트 안에서만 (종료 시 스냅샷 복구)
                self._cursor_visible_persist = False
            self.next_step()
        ...
        elif s_type == "GAME_OPTIONS_BUTTON":
            # 설정(option) 버튼 — 클릭 시 음악/새로시작/끝내기 메뉴
            from field_runtime import parse_step_bool, parse_step_persist, set_game_options_button_visible
            ...
            persist = parse_step_persist(step, default=False)
            set_game_options_button_visible(self, bool(visible), persist=bool(persist))
            self.next_step()
            ...
            def _spawn_named(target_name):
                ...
                from char_behavior import attach_npc_from_entry, spawn_as_mask_walker
                entry = {"name": target_name, "pos": list(pos_xy)[:2]}
                beh0 = step.get("behavior") or step.get("behavior_mode") or step.get("mode")
                if isinstance(beh0, dict):
                    entry["behavior"] = dict(beh0)
                elif isinstance(beh0, str) and beh0.strip():
                    entry["behavior"] = {"mode": beh0.strip()}
                    if step.get("radius") is not None:
                        entry["behavior"]["radius"] = step.get("radius")
                    if step.get("interval_ms") is not None:
                        entry["behavior"]["interval_ms"] = step.get("interval_ms")
                if spawn_as_mask_walker(target_name, entry):
                    ent = MaskWalkingCharacter(target_name, pos_xy, ch_info)
                else:
                    ent = BaseCharacter(target_name, pos_xy, ch_info)
                try:
                    attach_npc_from_entry(ent, entry)
                except Exception:
                    pass
            ...
            # 토큰 → 엔티티 목록 (없으면 스폰; all_npcs 는 기존만)
            place_targets = []
            seen_ids = set()
            for tok in tokens:
                if _is_all_npcs_token(tok):
                    for n in npcs or []:
                        ...
                        if eid in seen_ids:
                            continue
                        seen_ids.add(eid)
                        place_targets.append(n)
                    continue
                target_name = self._resolve_event_target_name(tok)
                if not target_name:
                    continue
                if target_name.strip().lower() == "player":
                    if player is not None and id(player) not in seen_ids:
                        seen_ids.add(id(player))
                        place_targets.append(player)
                    continue
                found = next(
                    (x for x in (npcs + objs) if getattr(x, "name", "") == target_name),
                    None,
                )
                if found is None:
                    found = _spawn_named(target_name)
                if found is not None and id(found) not in seen_ids:
                    seen_ids.add(id(found))
                    place_targets.append(found)
            ...
            do_fade = step.get("appear") == "fade"
            beh = step.get("behavior") or step.get("behavior_mode")
            spacing = _event_group_spacing_px(step)
            layout = _event_group_layout_mode(step)
            n_place = len(place_targets)

            for i, target in enumerate(place_targets):
                ...
                if is_remove:
                    if "dir" in step or "face" in step:
                        _event_apply_step_dir(target, step, player=player)
                    if not do_fade:
                        if target in objs:
                            objs.remove(target)
                        elif target in npcs:
                            ...
                            # player 등 씬 리스트에서 뺄 수 없는 대상: is_visible=False 로 숨김
                            # (스프라이트·발그림자 모두 draw 에서 스킵)
                ...
                if new_pos:
                    try:
                        bx, by = float(new_pos[0]), float(new_pos[1])
                        nx, ny = _event_group_pos(bx, by, i, n_place, spacing, layout=layout)
                        target.pos = [nx, ny]
                        if hasattr(target, "origin_pos"):
                            target.origin_pos = [nx, ny]
                        sm = getattr(target, "stop_moving", None)
                        if callable(sm):
                            sm()
                        try:
                            from char_behavior import set_roam_home
                            set_roam_home(target, nx, ny)
                        except Exception:
                            pass
                    except Exception:
                        pass

                if isinstance(beh, str) and beh.strip() and target is not player:
                    try:
                        from char_behavior import set_npc_behavior
                        kwargs = {}
                        if step.get("radius") is not None:
                            kwargs["radius"] = step.get("radius")
                        if step.get("interval_ms") is not None:
                            kwargs["interval_ms"] = step.get("interval_ms")
                        set_npc_behavior(target, beh.strip(), **kwargs)
                    except Exception:
                        pass
                elif isinstance(beh, dict) and beh.get("mode") and target is not player:
                    try:
                        from char_behavior import set_npc_behavior
                        set_npc_behavior(
                            target,
                            beh.get("mode"),
                            radius=beh.get("radius", step.get("radius")),
                            interval_ms=beh.get("interval_ms", step.get("interval_ms")),
                        )
                    except Exception:
                        pass

                # dir 는 pos/stop_moving/behavior 이후에 적용 (덮어쓰기 방지 + face_player 는 최종 좌표 기준)
                if "dir" in step or "face" in step:
                    # pos 없이 dir 만일 때도 이동 잔여 path 가 방향을 다시 돌리지 않게 정지
                    if not new_pos:
                        sm = getattr(target, "stop_moving", None)
                        if callable(sm):
                            try:
                ...
                # PLACE(등장/이동) 시 다시 보이게 — 이전 remove 로 숨겨진 player 복구
                if hasattr(target, "is_visible"):
            ...
            if do_fade:
                self._appear_batch_targets = list(place_targets)
                self.is_waiting_for_appear = True
                self.is_busy = True
            else:
                # PLACE persist — 즉시 완료 시 세이브 반영 (fade 는 _check_completion 에서)
                self._sync_place_persist(step, place_targets, player, is_remove=is_remove)
                self.next_step()

        elif s_type == "BEHAVIOR":
            # NPC ambient AI 모드 제어 — 다중 target / all_npcs
            targets = self._resolve_event_targets(
                step.get("target"), player, npcs, objs, allow_player=False
            )
            mode = (
                step.get("behavior")
            ...
            if targets and mode:
                try:
                    ...
                    if step.get("interval_ms") is not None:
                        kwargs["interval_ms"] = step.get("interval_ms")
                    nm = normalize_behavior_mode(mode)
                    for target in targets:
                        if target is player:
                            continue
                        set_npc_behavior(target, mode, **kwargs)
                        if nm in ("wander", "randomplay"):
                            set_roam_home(target)
                        # 이미 PLACE persist 된 NPC 면 behavior/follow 를 세이브에도 반영
                        self._sync_behavior_to_placed(target, mode, step)
                except Exception as e:
                    print(f"[BEHAVIOR] failed: {e}")
            self.next_step()

        elif s_type == "TUNE":
            # 이미 배치된 대상의 설정만 변경 — 다중 target / all_npcs
            ...
            # 월드 이펙트 — target 다중/all_npcs 시 각 위치에 스폰
            e_name = (step.get("name") or "").strip()
            if not e_name:
                self.next_step()
                return
            ...
            if targets:
                ...
                self.active_effects.append(new_effect)
                spawned.append(new_effect)
            if _parse_effect_step_wait(step) and (not is_loop) and spawned:
                self._effect_wait_refs = list(spawned)
                self._effect_wait_ref = spawned[-1]
            else:
                self._effect_wait_refs = None
            ...
            # 지정 좌표 + object_defs 애니 1회. pos 여러 곳이면 동시. 기본 wait.
            if not self._spawn_anim_once(step):
                self.next_step()
            elif not getattr(self, "_effect_wait_refs", None) and getattr(
                self, "_effect_wait_ref", None
            ) is None:
                self.next_step()
        ...
        # ==========================================================
            ...
            # 음악 재생(지정된 경우) — MUSIC_ASSETS 키 또는 파일명/경로
            if music:
                try:
                    path = None
                    if self.music:
                        lib = getattr(self.music, "library", None) or {}
                        if music not in lib:
                            try:
                                self.music.refresh_library()
                                lib = getattr(self.music, "library", None) or {}
                            except Exception:
                                pass
                        path = lib.get(music)
                    if path:
                        music = path
                    elif not os.path.isabs(music) and not music.replace("\\", "/").startswith("assets/"):
            ...
            self._mark_event_music_persist(step)
            self.next_step()
    ...
    def _mark_event_music_persist(self, step):
        """MUSIC_PLAY/STOP/END 가 이번 이벤트에서 BGM을 건드렸음을 기록.

        persist:true  → 이벤트 끝나도 지금 곡(또는 무음) 유지
        persist:false/생략 → 이벤트 종료 시 맵 field.music 으로 복귀
        같은 이벤트에서 여러 번 쓰면 마지막 MUSIC_* 의 persist 가 이긴다.
        """
        self._event_music_touched = True
        try:
            from field_runtime import parse_step_persist

            self._event_music_persist = bool(parse_step_persist(step, default=False))
        except Exception:
            raw = (step or {}).get("persist") if isinstance(step, dict) else None
            if isinstance(raw, str):
                self._event_music_persist = raw.strip().lower() in (
                    "1",
                    "true",
                    "t",
                    "yes",
                ...
                self._event_music_persist = bool(raw)
    ...
    def _check_completion(self, player, camera, npcs, objs, dt_sec=1.0 / 60.0):
        ...
        if getattr(self, 'is_waiting_for_appear', False):
            batch = getattr(self, "_appear_batch_targets", None)
            if isinstance(batch, list) and batch:
                targets = [t for t in batch if t is not None]
            else:
                targets = self._resolve_event_targets(step.get("target"), player, npcs, objs)
            if not targets:
                self.is_waiting_for_appear = False
                self._appear_batch_targets = None
                self.next_step()
                return
            is_remove = (step.get("action") == "remove")
            try:
                ...
                if is_remove:
                    for target in targets:
                        if target in objs:
                            ...
                            # player 등: 페이드 퇴장 완료 후 is_visible=False (그림자 포함 숨김)
                            if hasattr(target, "is_visible"):
                                target.is_visible = False
                # PLACE fade 완료 후 persist/remove 세이브 반영
                if s_type == "PLACE":
                    try:
                        self._sync_place_persist(
                            step, targets, player, is_remove=bool(is_remove)
                        )
                    except Exception as e:
                        print(f"[PLACE fade persist] {e}")
                self.is_waiting_for_appear = False
                self._appear_batch_targets = None
                self.next_step()
            return
        ...
        if s_type == "CHANGE":
            ...
            if not cf:
                ...
                return
            item = cf["item"]
            sec = max(0.05, float(cf.get("sec") or 0.05))
            cf["elapsed"] = float(cf.get("elapsed") or 0.0) + float(dt_sec)
            u = min(1.0, float(cf["elapsed"]) / sec)
            try:
                start_a = int(cf.get("start_alpha", 255) or 255)
            except (TypeError, ValueError):
                start_a = 255
            start_a = max(0, min(255, start_a))
            item.alpha = int(round(start_a * u))
            ghost = getattr(item, "_change_ghost", None)
            if isinstance(ghost, dict):
                ghost["alpha"] = int(round(start_a * (1.0 - u)))
            if u >= 1.0:
                item.alpha = start_a
                _clear_entity_change_ghost(item)
        ...
        if s_type in ("EFFECT", "ANIM_ONCE"):
            refs = getattr(self, "_effect_wait_refs", None)
            if isinstance(refs, list) and refs:
                pending = [
                    r
                    for r in refs
                    if r is not None
                    and (not bool(getattr(r, "is_done", False)))
                    and r in self.active_effects
                ]
                if not pending:
                    self._effect_wait_refs = None
                    self._effect_wait_ref = None
        ...
        if s_type == "MOVE":
            ...
            move_targets = self._resolve_event_targets(step.get("target"), player, npcs, objs)
            wps = _normalize_move_step_waypoints(step)
            if not wps or not move_targets:
                # 대상/경로 없으면 연출 스킵하고 진행 (dir-only 는 execute 에서 이미 next)
                self.next_step()
                return
            spacing = _event_group_spacing_px(step)
            layout = _event_group_layout_mode(step)
            n_tg = len(move_targets)
            all_arrived = True
    ...
    def _apply_event_result_to_save(self, results, event_id=None, *, source="end", session=False):
        """이벤트 result / RESULT 스텝 patch → flow.save_data 또는 session_progress.

        source:
          - "end": 이벤트·CALL_EVENT 종료 시 헤더 result
          - "step": RESULT 이벤트 스텝 (중간 적용)
        session=True: 세이브 파일에 안 씀. 이번 실행만 (휘발 마커·미완료 점유).
          이후 같은 키를 일반 RESULT 로 쓰면 세션 값을 지우고 세이브에 확정한다.
        """
        if not results or not getattr(self, "flow", None):
            return
        eid = event_id or self.active_event_id
        if session:
            if not isinstance(getattr(self.flow, "session_progress", None), dict):
                self.flow.session_progress = {}
            dest = self.flow.session_progress
            tag = "세션"
        else:
            dest = self.flow.save_data
            tag = "세이브"
        written = []
        new_prog = results.get("mainprogress", None)
        if new_prog is not None and str(new_prog) != "":
            dest["mainprogress"] = new_prog
            written.append("mainprogress")
            print(f"[진행도 변경{'' if not session else '·세션'}] -> {new_prog}")
        if "add_laugh_point" in results:
            # 웃음 포인트는 세션이어도 세이브에 가산 (휘발이면 포인트만 날아가 버림)
            points_to_add = results["add_laugh_point"]
            laugh_dest = self.flow.save_data
            current_points = laugh_dest.get("laugh_point", 0)
            laugh_dest["laugh_point"] = current_points + points_to_add
            print(f"[포인트 획득] +{points_to_add} (총: {laugh_dest['laugh_point']})")
        for rk, rv in results.items():
            if rk in ("mainprogress", "add_laugh_point", "gamestart", "session"):
                continue
            ...
            if isinstance(rk, str) and rk.startswith("+="):
                ...
                try:
                    cur = dest.get(base)
                    if cur is None and not session:
                        cur = 0
                    elif cur is None:
                        cur = self.flow.save_data.get(base, 0)
                    dest[base] = float(cur or 0) + float(rv)
                    if float(dest[base]) == int(dest[base]):
                        dest[base] = int(dest[base])
                except (TypeError, ValueError):
                    dest[base] = rv
                written.append(base)
                print(f"[{tag} 가산] {base} += {rv} → {dest.get(base)}")
                continue
            dest[rk] = rv
            written.append(rk)
            print(f"[{tag} 갱신] {rk} = {rv}")
        if not session:
            sp = getattr(self.flow, "session_progress", None)
            if isinstance(sp, dict):
                for k in written:
                    sp.pop(k, None)
        if eid:
            if str(source or "end") == "step":
                print(f"[RESULT 스텝{'·session' if session else ''}] ID: {eid}")
            else:
    ...
    def end_event(self):
        ...
        if self._event_call_stack:
            ...
            self._say_ui_fade_phase = None
            self._loop_end_to_head, self._loop_pairs = _parse_loop_jump_table(self.active_event)
            self._event_skip_depth = int(frame.get("skip_depth", 0) or 0)
            self._event_if_taken = bool(frame.get("if_taken", False))
        ...
        for ent in getattr(self, "_active_entities", []) or []:
            try:
                ...
                pass
            # ACTION_ANIM jump loop hop 정리 (persist 는 유지)
            try:
                if getattr(ent, "_hop_repeat", None) and not bool(
                    getattr(ent, "_action_anim_persist", False)
                ):
                    ent._hop_repeat = None
                    sm = getattr(ent, "stop_moving", None)
                    if callable(sm):
                        sm()
            except Exception:
                pass
        ...
        self._effect_wait_refs = None
        self._appear_batch_targets = None
        # 최상위 이벤트 종료 → 세이브 가드 해제 (이후 종료 시 현재 위치 저장 가능)
        try:
            if self.flow and hasattr(self.flow, "clear_pre_event_checkpoint"):
                self.flow.clear_pre_event_checkpoint()
        except Exception:
            pass
        # CHANGE 크로스페이드가 끝나기 전에 이벤트가 종료되면 외형 교체를 마저 적용하고 alpha 복구
        cf_end = getattr(self, "_change_fade", None)
        if cf_end:
            try:
                item_end = cf_end.get("item")
                if item_end is not None:
                    if not cf_end.get("applied"):
                        self._apply_change_retarget(
                            item_end,
                            cf_end["new_name"],
                            persist=bool(cf_end.get("persist")),
                            player=getattr(self, "_runtime_player", None),
                            npcs=getattr(self, "_runtime_npcs", None),
                            objs=getattr(self, "_runtime_objs", None),
                        )
                    try:
                        item_end.alpha = int(cf_end.get("start_alpha", 255) or 255)
                    except (TypeError, ValueError):
                        item_end.alpha = 255
                    _clear_entity_change_ghost(item_end)
            except Exception:
                pass
            self._change_fade = None
        self.active_effects = []
        ...
        self.world_zoom_step_speed = None
        self.world_zoom_timed = None
        # 커서: persist 없으면 이벤트 시작 직전 상태로 복구 (무조건 True 강제 금지).
        # persist:true 인 CURSOR_VISIBLE 이 있었으면 현재 값 유지.
        if not bool(getattr(self, "_cursor_visible_persist", False)):
            self.cursor_visible = bool(getattr(self, "_cursor_visible_snapshot", True))
        self._cursor_visible_persist = False
        ...
        if not bool(getattr(self, "_game_options_button_persist", False)):
            from field_runtime import set_game_options_button_visible

            set_game_options_button_visible(
                self,
                bool(getattr(self, "_game_options_button_visible_snapshot", False)),
                persist=False,
            )
        self._commit_persisted_entity_event_zooms()
        self._restore_entity_event_visuals()
        # BGM: MUSIC_* 를 썼고 persist 가 아니면 맵 기본 음악으로 복귀
        try:
            touched = bool(getattr(self, "_event_music_touched", False))
            keep = bool(getattr(self, "_event_music_persist", False))
            self._event_music_touched = False
            self._event_music_persist = False
            if touched and not keep:
                from field_runtime import restore_map_default_music

                restore_map_default_music(self, map_id=getattr(self, "_runtime_map_id", None))
        except Exception as e:
            print(f"[MUSIC] restore map default failed: {e}")
    ...
    def _execute_entity_fx_step(self, step, player, npcs, objs):
        """ENTITY_FX — 대상 캐릭터/오브젝트 반짝임·틴트·zoom. 다중 target / all_npcs."""
        action = (step.get("action") or step.get("cmd") or "start").strip().lower()
        if action in ("stop", "clear", "off", "remove", "end"):
            tgt_name = (step.get("target") or step.get("who") or "").strip()
            if tgt_name:
                for ent in self._resolve_event_targets(tgt_name, player, npcs, objs):
        ...
        if isinstance(on_raw, str) and on_raw.strip().lower() in ("0", "false", "f", "no", "n", "off"):
            ...
            return
        tgt = step.get("target") or step.get("who") or "player"
        ents = self._resolve_event_targets(tgt, player, npcs, objs, default_to_player=True)
        if not ents:
            print(f"[FX entity_fx] 대상 없음: {tgt!r}")
            self.next_step()
            return
        for ent in ents:
            self._track_event_entity_visual(ent, step)
    ...
    def _screen_timing_from_step(self, step) -> dict:
        ...
        return {
            ...
            "force": bool(force),
            "auto": bool(auto),
            # hi_res 는 무시. 예전엔 NATIVE_640 전환이었으나 이미지는 현재 논리 화면에 스케일.
            "hi_res": False,
        }
