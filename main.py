...
from flow import (
    ...
    resolve_continue_spawn,
    collect_active_zone_block_rects,
...
def clear_main_ui_font_cache() -> None:
    try:
...
def get_ui_font(size: int, slot: str = "ui"):
    """한글 포함 UI용 폰트 — 용도 슬롯(UI_FONT_PROFILES) 스타일 적용.

    slot 예: ui / hud_racing / hud_baseball / hud_fishing / logo …
    """
    try:
        size_i = int(size)
    except Exception:
        size_i = 14
    size_i = max(6, min(256, size_i))
    sid = str(slot or "ui").strip() or "ui"
    try:
        from engine import resolve_font_profile

        return resolve_font_profile(sid, size_px=size_i)
    except Exception:
        pass
    key = (sid, int(size_i))
    ...
    if fp:
        try:
            from engine import OutlinedUIFont, get_font_profile

            f = OutlinedUIFont(
                pygame.font.Font(fp, size_i),
                font_key="default",
                style=get_font_profile(sid),
            )
    ...
    try:
        ...
        f = OutlinedUIFont(
            pygame.font.SysFont("malgungothic", size_i),
            font_key="default",
            style=get_font_profile(sid),
        )
    except Exception:
        from engine import OutlinedUIFont
...
def activity_font_fn(activity_id: str | None = None):
    """미니게임별 HUD 슬롯에 묶인 font_fn."""
    aid = str(activity_id or "").strip().lower()
    slot = {
        "racing": "hud_racing",
        "baseball": "hud_baseball",
        "fishing": "hud_fishing",
    }.get(aid, "ui")

    def _fn(size: int, _slot=slot):
        return get_ui_font(size, _slot)
...
# --- 다운/프리즈 원인 추적 (data.py CONFIG — DIAG_TRACE 등) ---
def _diag_cfg(key, default=None):
    try:
...
_diag_cp = "boot"
_diag_loop_i = 0
_diag_boot_done_at = None  # loop_i when field_boot ended
_diag_stop_reason = None
_diag_cp_t = 0.0  # last mark wall time
_diag_frame_t0 = 0.0
_diag_sections = []  # [(name, dt_ms), ...] current frame
_diag_last_frame_sections = []
_diag_last_loop_top_t = 0.0


def _diag(msg: str) -> None:
    """콘솔 + logs/runtime.log 즉시 flush (마지막 찍힌 줄 = 멈춘 지점)."""
    if not bool(_diag_cfg("DIAG_TRACE", False)):
        return
    line = f"[DIAG] {_diag_cp} | {msg}"
    try:
...
def _diag_mark(cp: str, detail: str = "", *, announce: bool = False) -> None:
    """체크포인트 갱신 + 구간 소요시간 누적. announce/detail 있을 때만 즉시 출력."""
    global _diag_cp, _diag_cp_t, _diag_sections
    now = time.perf_counter()
    prev = str(_diag_cp or "?")
    if _diag_cp_t > 0.0:
        dt_ms = (now - _diag_cp_t) * 1000.0
        _diag_sections.append((prev, dt_ms))
        # 단일 구간이 길면 즉시 찍기 (다음 하트비트 전에 원인 남김)
        if dt_ms >= float(_diag_cfg("DIAG_SLOW_FRAME_SEC", 1.5)) * 1000.0:
            _diag(f"SLOW_SECTION {prev}→{cp} {dt_ms:.0f}ms")
    _diag_cp = str(cp or "?")
    _diag_cp_t = now
    if announce or detail:
        _diag(detail or "mark")


def _diag_begin_frame() -> None:
    """루프 상단: 이전 프레임 갭/구간 덤프 후 새 프레임 시작."""
    global _diag_frame_t0, _diag_sections, _diag_last_frame_sections, _diag_last_loop_top_t, _diag_cp_t
    now = time.perf_counter()
    if _diag_last_loop_top_t > 0.0:
        gap = now - _diag_last_loop_top_t
        if gap >= float(_diag_cfg("DIAG_SLOW_FRAME_SEC", 1.5)):
            parts = ",".join(f"{n}:{dt:.0f}" for n, dt in (_diag_last_frame_sections or [])[-12:])
            _diag(
                f"SLOW_GAP {gap*1000.0:.0f}ms i={_diag_loop_i} "
                f"last_cp={_diag_cp} sections=[{parts}]"
            )
    _diag_last_loop_top_t = now
    _diag_last_frame_sections = list(_diag_sections)
    _diag_sections = []
    _diag_frame_t0 = now
    _diag_cp_t = now


def _diag_atexit() -> None:
    try:
        parts = ",".join(f"{n}:{dt:.0f}" for n, dt in (_diag_sections or _diag_last_frame_sections or [])[-16:])
        _diag(
            f"atexit stop_reason={_diag_stop_reason!r} last_cp={_diag_cp} "
            f"loop_i={_diag_loop_i} sections=[{parts}]"
        )
    except Exception:
        pass
...
def _screen_to_world_from_render_xform(mx, my, *, xf):
    ...
    순서: 월드 후처리 줌 해제 → (Mode7이면 즉시 월드) 또는 틸트/쉬어 → cam_draw.
    """
    if not xf:
        return None
    try:
    ...
    # Mode7: world_surf 좌표 → 월드 직행 (틸트/쉬어/cam_draw 경로와 혼합하지 않음)
    if bool(xf.get("rotate3d_on", False)) and bool(xf.get("rotate3d_mode7", False)):
        ctx = xf.get("mode7_ctx")
        if isinstance(ctx, dict):
            try:
                from engine import rotate3d_mode7_screen_to_world

                return rotate3d_mode7_screen_to_world(mx, my, ctx)
            except Exception:
                pass
...
def _player_feet_screen_xy_like_draw(px, py, cam_draw_x, cam_draw_y, z, y_transform, x_offset_fn, x_scale_fn=None, x_shift_fn=None, pivot_xy=None, cam_angle_rad=0.0, mode7_ctx=None, view_w=None):
    """
    FieldItem.draw와 동일: cam_draw(렌더 스냅·쉬어 중앙 보정 반영) + 3D_ROTATE/틸트 좌표 보정.
    cam.to_screen(cam.pos 기준)과 달라서, 월드 후처리 줌 앵커는 반드시 이 경로를 써야 쉬어 시 튐이 없다.
    """
    from engine import map_feet_screen_xy, rotate3d_mode7_entity_screen

    if mode7_ctx:
        return rotate3d_mode7_entity_screen(
            float(px),
            float(py),
            mode7_ctx,
            height_off=0.0,
    ...
    flat_x = (float(px) - float(cam_draw_x)) * float(z)
    flat_y = (float(py) - float(cam_draw_y)) * float(z)
    try:
...
def _save_game_with_activity_anchor(
        ...
        if ret_map:
            if ret_pos is not None and len(ret_pos) >= 2:
                flow.save_game(
                    ...
                    npcs=npcs,
                    event_followers=(
                        list(getattr(ev_mgr, "_followers", None) or [])
    ...
    if ev_mgr is not None:
        try:
            _fol = list(getattr(ev_mgr, "_followers", None) or [])
        except Exception:
            _fol = None
    flow.save_game(
        map_id,
        ...
        objs=objs,
        npcs=npcs,
        event_followers=_fol,
    )


def _restore_followers_from_save(ev_mgr, flow, map_id, *, objs=None, npcs=None, player=None):
    """save_data.event_followers / placed → ev_mgr._followers + follow behavior."""
    try:
        from flow import restore_event_followers_from_placed

        restore_event_followers_from_placed(
            ev_mgr,
            flow.save_data,
            map_id,
            objs=objs,
        ...
        print(f"[followers] restore failed: {e}")
...
def _hold_global_fade_black(ev_mgr) -> None:
    """데모 종료 암전을 유지한다. 밝히기는 본편 첫 이벤트 FADEIN에 맡긴다."""
    try:
        ev_mgr._pending_fade_in_after_fadeout_sec = None
        ev_mgr.fade_alpha = 255
        ev_mgr.fade_target = 255
        ev_mgr.is_fading = False
        ev_mgr.fade_duration_sec = 0.0
        ev_mgr.fade_elapsed_sec = 0.0
    except Exception:
        pass
...
def _spawn_new_game_after_char_select(flow, field_activities, ev_mgr, *, cam=None):
    """
    캐릭터 선택 완료 후 본편 스폰.
    NEW_GAME_SPAWN_* 로 load_map → 세이브 → boot_phase=2.
    화면은 검정으로 유지한다. 밝히기는 본편 첫 auto 이벤트(ev_gl_post_demo_*)의 FADEIN에 맡긴다.
    ...
    if cam is not None:
        try:
            ...
            pass
    _hold_global_fade_black(ev_mgr)
    print(
...
def _should_start_racing_exit_event(act_res):
    return (
        isinstance(act_res, dict)
...
def _queue_racing_exit_transition(flow, act_res):
    target_map = str(act_res.get("return_map") or "").strip()
    target_pos = act_res.get("return_pos")
    if not target_map:
        return False
...
def _try_start_racing_exit_event(
    flow,
    ev_mgr,
    events_catalog,
    act_res,
    *,
    ...
    if not _should_start_racing_exit_event(act_res):
        return False
    if not _queue_racing_exit_transition(flow, act_res):
        return False
    from flow import start_system_event

    event_id = str(act_res.get("exit_event_id") or "ev_racing_exit").strip()
    if not start_system_event(
        ev_mgr,
        events_catalog,
        event_id,
        ...
        flow.save_data.pop("racing_exit_map", None)
        flow.save_data.pop("racing_exit_pos", None)
        return False
    try:
        ev_mgr.remove_ui_overlay("racing_exit")
    except Exception:
        pass
    print(f"[racing] exit event started: {event_id}")
    return True
    ...
    *,
    ...
    if isinstance(act_res.get("save_patch"), dict) and act_res.get("save_patch"):
        try:
            ...
            pass
    # 첫 시작 캐릭터 선택 → 페이드아웃 후 본편 스폰 (밝히기는 첫 auto 이벤트)
    if (
        ...
        try:
            ...
            pass
        try:
            fade_sec = float(CONFIG.get("CHAR_SELECT_EXIT_FADEOUT_SEC", 1.0) or 1.0)
        except Exception:
            fade_sec = 1.0
        fade_sec = max(0.0, fade_sec)
        try:
            fa = int(getattr(ev_mgr, "fade_alpha", 0) or 0)
            already_black = (not bool(getattr(ev_mgr, "is_fading", False))) and fa >= 250
        except Exception:
            already_black = False
        if already_black or fade_sec <= 0.0:
            return _spawn_new_game_after_char_select(flow, field_activities, ev_mgr, cam=cam)
        try:
            ev_mgr.start_global_fade_to(255, fade_sec)
        except Exception:
            return _spawn_new_game_after_char_select(flow, field_activities, ev_mgr, cam=cam)
...
def main():
    global _diag_cp, _diag_loop_i, _diag_boot_done_at, _diag_stop_reason
    atexit.register(_diag_atexit)
    _diag_mark("main_enter", announce=True)
    log_line("=== launch ===")
    ...
    def _apply_map_field_visuals(target_map_id, *, instant=True):
        ...
        # world_data[map].field — 맵별 틸트/쉬어/물결 타일 기본값 (없으면 CONFIG)
        apply_map_field_defaults(
            target_map_id,
            ui,
            ev_mgr=ev_mgr,
            world_data=flow.world_data,
            wave_ambient=wave_fx,
        )
        if instant:
    ...
    def _fishing_input_lock_active() -> bool:
        try:
            if str(field_activities.active_id or "").strip().lower() == "fishing":
                return True
        except Exception:
            pass
        try:
            for ov in list(getattr(ev_mgr, "_ui_overlays", None) or []):
                if ov.get("id") == "fishing_exit" and ov.get("phase") != "done":
                    return True
        except Exception:
            pass
        return False
    ...
    running = True
    _diag_mark("loop_ready", "entering while running", announce=True)

    def _request_stop(reason: str) -> None:
        """running=False 로 빠지는 모든 경로 — 원인 문자열을 남긴다."""
        nonlocal running
        global _diag_stop_reason
        _diag_stop_reason = str(reason or "unknown")
        _diag(f"STOP → running=False reason={_diag_stop_reason}")
        running = False

    def _diag_heartbeat(tag: str = "hb") -> None:
        try:
            ae = getattr(ev_mgr, "active_event_id", None)
            fa = int(getattr(ev_mgr, "fade_alpha", 0) or 0)
            ft = int(getattr(ev_mgr, "fade_target", 0) or 0)
            fading = bool(getattr(ev_mgr, "is_fading", False))
            bp = int(getattr(flow, "boot_phase", -1) or -1)
            fbr = (flow.save_data or {}).get("field_boot_ready")
            px = int(player.pos[0]) if player is not None else -1
            py = int(player.pos[1]) if player is not None else -1
            mid = str(map_id)
        except Exception as e:
            _diag(f"{tag} state-read-fail: {e}")
            return
        _diag(
            f"{tag} i={_diag_loop_i} map={mid} pos=({px},{py}) "
            f"boot={bp} field_boot_ready={fbr} "
            f"evt={ae!r} fade={fa}->{ft} fading={fading}"
        )
    ...
    def _clear_transform_caches(*, run_gc: bool = True):
        ...
        nonlocal rotate3d_tmp, rotate3d_mask_tmp
        nonlocal _render_cache_mb
        try:
            _render_cache.clear()
        except Exception:
    ...
    while running:
        ...
        try:
            ...
            loop_i = 1
        _diag_loop_i = int(loop_i)
        _diag_begin_frame()
        _diag_mark("loop_top")
        # field_boot 직후·평상시 하트비트 (마지막 줄이 멈춘 프레임)
        try:
            _after_boot = (
                _diag_boot_done_at is not None
                and (loop_i - int(_diag_boot_done_at)) <= int(_diag_cfg("DIAG_VERBOSE_FRAMES_AFTER_BOOT", 240))
            )
            _hb_every = 5 if _after_boot else int(_diag_cfg("DIAG_HEARTBEAT_EVERY", 30))
            if bool(_diag_cfg("DIAG_TRACE", False)) and (loop_i <= 3 or loop_i % max(1, _hb_every) == 0):
                _diag_heartbeat("hb")
        except Exception as _diag_e:
            _diag(f"heartbeat-fail: {_diag_e}")
        ...
        if field_activities.is_active and not ev_mgr.active_event:
            ...
            if _btt is not None:
                try:
                    ...
                    pass
            # 레이스 등 — Mode7 strength 오버라이드
            _r3t = getattr(_sess, "field_rotate3d_target", None)
            if _r3t is not None:
                try:
                    ui.rotate3d_target = max(0.0, min(1.0, float(_r3t)))
                    ui.rotate3d_on = float(ui.rotate3d_target) > 0.05
                except (TypeError, ValueError):
                    pass
        ...
        try:
            rotate3d_speed = float(CONFIG.get("ROTATE3D_SPEED", 0.14) or 0.14)
        except Exception:
            rotate3d_speed = 0.14
        rotate3d_speed = max(0.0, min(1.0, rotate3d_speed))
        try:
            rotate3d_eps = float(CONFIG.get("ROTATE3D_EPS", 0.003) or 0.003)
        except Exception:
            rotate3d_eps = 0.003

        if isinstance(getattr(ev_mgr, "rotate3d_control", None), dict):
            rc = ev_mgr.rotate3d_control
            try:
                ui.rotate3d_target = float(rc.get("target", 0.0))
            except (TypeError, ValueError):
                ui.rotate3d_target = 0.0
            ui.rotate3d_target = max(0.0, min(1.0, float(ui.rotate3d_target)))
            if rc.get("instant_once"):
                rotate3d_current = float(ui.rotate3d_target)
                rc["instant_once"] = False

        _r3_tc = getattr(ev_mgr, "rotate3d_control", None)
        if isinstance(_r3_tc, dict) and _r3_tc.get("duration_sec") is not None:
            try:
                _r3_dur = float(_r3_tc.get("duration_sec"))
            except (TypeError, ValueError):
                _r3_dur = None
            if _r3_dur is not None and _r3_dur > 0.0 and "start" not in _r3_tc:
                timed_effect_init(
                    _r3_tc,
                    float(rotate3d_current),
                    float(ui.rotate3d_target),
                    _r3_dur,
                    now_ms=_r3_tc.get("t0_ms"),
                )
            if _r3_dur is not None and _r3_dur > 0.0:
                rotate3d_current = timed_effect_value(_r3_tc, float(rotate3d_current))
            elif abs(float(ui.rotate3d_target) - float(rotate3d_current)) <= rotate3d_eps:
                rotate3d_current = float(ui.rotate3d_target)
            else:
                rotate3d_current = float(ui.rotate3d_target)
        elif abs(float(ui.rotate3d_target) - float(rotate3d_current)) <= rotate3d_eps:
            ...
            alpha = visual_smooth_step(float(rotate3d_speed), dt_visual_sec)
            rotate3d_current = float(rotate3d_current) + (float(ui.rotate3d_target) - float(rotate3d_current)) * alpha

        # 3D_ROTATE 시험: < > 키로 시점 회전 (Mode7 cam_angle)
        # 레이스 등 activity 가 mode7_cam_heading 을 소유하면 핫키 회전 무시
        _r3_heading_owned = False
        try:
            _fa_h = getattr(getattr(field_activities, "_session", None), "mode7_cam_heading", None)
            _r3_heading_owned = _fa_h is not None
        except Exception:
            _r3_heading_owned = False
        if abs(float(rotate3d_current)) > float(rotate3d_eps) and (not _r3_heading_owned):
            try:
                _r3_ang_spd = float(CONFIG.get("ROTATE3D_ANGLE_SPEED", 2.2) or 2.2)
            except (TypeError, ValueError):
                _r3_ang_spd = 2.2
            _r3_ang_spd = max(0.05, min(12.0, float(_r3_ang_spd)))
            _r3_keys = pygame.key.get_pressed()
            _r3_turn_l = bool(_r3_keys[pygame.K_COMMA])
            _r3_turn_r = bool(_r3_keys[pygame.K_PERIOD])
            try:
                if hasattr(pygame, "K_LESS"):
                    _r3_turn_l = _r3_turn_l or bool(_r3_keys[pygame.K_LESS])
                if hasattr(pygame, "K_GREATER"):
                    _r3_turn_r = _r3_turn_r or bool(_r3_keys[pygame.K_GREATER])
            except Exception:
                pass
            if _r3_turn_l:
                ui.rotate3d_angle = float(getattr(ui, "rotate3d_angle", 0.0)) - _r3_ang_spd * float(dt_visual_sec)
            if _r3_turn_r:
                ui.rotate3d_angle = float(getattr(ui, "rotate3d_angle", 0.0)) + _r3_ang_spd * float(dt_visual_sec)
        ...
        # --- 외부 minigames/ 는 본체 비포함(no-op 훅만 호환) ---
        ...
        if _fa_sess is not None:
            try:
                field_activities.tick(dt_visual_sec, player, int(pygame.time.get_ticks()), npcs=npcs, objs=objs)
            except Exception:
                pass
        ...
        _diag_mark("after_ev_update")
        try:
            _ended = getattr(ev_mgr, "last_ended_event_id", None)
            if _ended == "ev_gl_field_boot" or (
                _ended and str(_ended).endswith("field_boot")
            ):
                if _diag_boot_done_at is None:
                    _diag_boot_done_at = int(_diag_loop_i)
                    _diag_heartbeat("field_boot_ended")
        except Exception as _e:
            _diag(f"field_boot_watch_fail: {_e}")
        # 캐릭터 선택 페이드아웃 완료 → 검정 유지한 채 본편 스폰 (밝히기는 첫 auto 이벤트)
        if getattr(flow, "pending_char_select_spawn_after_fade", False):
            try:
                fading = bool(getattr(ev_mgr, "is_fading", False))
                fa = int(getattr(ev_mgr, "fade_alpha", 0) or 0)
            except Exception:
                fading, fa = False, 255
            if (not fading) and fa >= 250:
                loaded = _spawn_new_game_after_char_select(
                    flow, field_activities, ev_mgr, cam=cam
                )
                if loaded is not None:
                    map_id, bg, mask, player, objs, npcs = loaded
                    _sync_map_bg_size()
        ...
        if ev_mgr.pending_map_change:
            ...
            try:
                ...
                snapshot_placed_from_live(
                    ...
                    objs,
                    npcs,
                    event_followers=list(getattr(ev_mgr, "_followers", None) or []),
                )
            except Exception:
                pass
        ...
        for event in pygame.event.get():
            ...
            if ev_mgr.active_event:
                # 외부 minigames/ 비사용 — 활성일 때만 입력 가로채기(항상 False)
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if ev_mgr.try_escape_click(event.button):
                        if not getattr(ev_mgr, "_escape_preserve_locomotion", False):
                            player.stop_moving()
                            for n in npcs:
                ...
                if field_activities.is_active:
                    ...
                    if event.type == pygame.KEYDOWN and _is_primary_action_key(event.key):
                        ...
                        if _pac == "quit":
                            _request_stop("primary_key_in_event:game_exit_yes")
                            continue
                        if _pac == "consumed":
                            continue
                continue
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                ...
                # 필드 활동/낚시 종료 오버레이 중 이동 클릭 무시
                if field_activities.blocks_field_move() or _fishing_input_lock_active():
                    continue
                ...
                if not is_double:
                    try:
                        if str(getattr(player, "_move_mode", "walk")) == "run" and bool(getattr(player, "path", None)):
                            move_mode = "run"
                    except Exception:
                        move_mode = "walk"
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if _is_primary_action_key(event.key):
                    _pac = _primary_action_at_cursor()
                    if _pac == "quit":
                        _request_stop("primary_key:game_exit_yes")
                        continue
                    if _pac == "consumed":
                        continue
        ...
        if ev_mgr.last_ended_event_id == demo_id:
            ...
            if (
                ...
                if not resolved:
                    resolved = {
                        "current_map": save_spawn_snapshot["current_map"],
                        "player_pos": list(
                            save_spawn_snapshot.get("player_pos")
                            or flow.world_data.get(
                                save_spawn_snapshot["current_map"], {}
                            ).get("start_pos", [100, 100])
                        ),
                    }
                flow.save_data["current_map"] = resolved["current_map"]
                spawn_sd = {
                    "current_map": resolved["current_map"],
                    "player_pos": list(resolved["player_pos"]),
                }
                ...
                _queue_sync_for_map(map_id)
                _apply_map_field_visuals(map_id, instant=True)
                # 세이브 쓰기 전에 동행 복원 — 빈 _followers 로 event_followers 를 덮어쓰지 않음
                _restore_followers_from_save(
                    ev_mgr, flow, map_id, objs=objs, npcs=npcs, player=player
                )
                _save_game_with_activity_anchor(
                    flow, field_activities, map_id, player.pos, objs=objs, npcs=npcs, ev_mgr=ev_mgr
                )
                # 데모가 이미 페이드아웃. 캐릭터 선택 건너뛰면 밝히지 않고 첫 이벤트 FADEIN까지 암전 유지.
                _hold_global_fade_black(ev_mgr)
                _diag_heartbeat("after_continue_spawn_hold_black")
                print(
                    f"[온보딩 완료] 이어하기 캐릭터={flow.save_data.get('player_char')} "
                    f"스폰 맵={map_id}, pos={flow.save_data['player_pos']} "
                    f"mode={CONFIG.get('CONTINUE_SPAWN_MODE', 'save')}"
                )
            else:
        ...
        for _sim_i in range(max(0, int(sim_steps))):
            ...
            try:
                _zone_blocks = collect_active_zone_blocks(
                    flow.world_data,
                    map_id,
                    flow.save_data,
                    session_vars=eval_session_vars(flow),
                ...
                _zone_blocks = []
            try:
                player._event_zone_blocks = _zone_blocks
            except Exception:
                pass
            for _n in npcs or []:
                try:
                    _n._event_zone_blocks = _zone_blocks
                except Exception:
                    pass
            ...
            if swing_ride_mode in ("mount", "ride"):
                ...
                try:
                    ...
                    try:
                        ...
                        aid = ""
                    if aid in ("racing", "bullfrog", "lotus_cross"):
                        player.path = []
                        player.target = list(player.pos)
                    else:
                        player.stop_moving()
        ...
        _diag_mark("after_sim_moves")
        # BLOCK 벽 접촉 안내 대사 (이동이 존에 막혔을 때 1회)
        try:
            try_trigger_zone_block_say(
                flow,
                ev_mgr,
        ...
        if app_force_quit_has_candidate(keys) and app_force_quit_combo_pressed(keys):
            _request_stop("app_force_quit_combo")
            continue
        ...
        _diag_mark("after_cam_update")
        # SAY 회피/상단 폴백: cam.update 이후 맵 끝·발 위치로 다시 판정
        try:
            if bool(getattr(ev_mgr, "is_talking", False)):
                ev_mgr._refresh_say_cam_avoid()
        except Exception:
            pass
        ...
        # Mode7 활성 시: 필드 follow 카메라와 Mode7 카메라 이원화 방지 — cam은 플레이어에 고정(앵커만).
        # 실제 시야/heading 은 ui.rotate3d_angle(+ activity) → Mode7 cam_heading 만 사용.
        if rotate3d_active:
            try:
                cam.pos[0] = float(player.pos[0])
                cam.pos[1] = float(player.pos[1])
            except Exception:
                pass
        if perf_enabled and t0 is not None:
        ...
        _diag_mark("render_begin")
        t_render0 = _pnow() if perf_enabled else None
        now_s = time.time()
        rotate3d_cfg = engine_mod.rotate3d_config_from_data() if rotate3d_active else None
        rotate3d_horizon = 0
        rotate3d_cam_angle = float(getattr(ui, "rotate3d_angle", 0.0) or 0.0)
        rotate3d_mode7_ctx = None
        x_scale_fn = None
        x_shift_fn = None
        ...
        # Mode7: 일반 크롭/틸트/쉬어 배경을 그리지 않는다(아래에서 Mode7만 샘플링).
        if rotate3d_active:
            bg_dx = 0
            bg_dy = 0
        ...
        if rotate3d_active and rotate3d_cfg is not None and bg is not None:
            ...
            lw_i = max(1, int(lw_i))
            lh_i = max(1, int(lh_i))
            if rotate3d_tmp is None or rotate3d_tmp.get_width() != lw_i or rotate3d_tmp.get_height() != lh_i:
                rotate3d_tmp = pygame.Surface((lw_i, lh_i))
            try:
                px_w = float(player.pos[0])
                py_w = float(player.pos[1])
            except Exception:
                px_w, py_w = 0.0, 0.0
            # heading: activity mode7_cam_heading 우선, 없으면 base + ui.rotate3d_angle
            base_heading = float(rotate3d_cfg.get("base_heading", math.pi * 0.5))
            cam_heading = base_heading + float(rotate3d_cam_angle)
            try:
                _fa_sess_m7 = getattr(field_activities, "_session", None)
                _m7h = getattr(_fa_sess_m7, "mode7_cam_heading", None)
                if _m7h is not None:
                    cam_heading = float(_m7h)
            except (TypeError, ValueError):
                pass
            mode7_cfg = rotate3d_cfg
            try:
                _m7x = getattr(_fa_sess_m7, "mode7_player_x_frac", None)
                if _m7x is not None:
                    mode7_cfg = dict(rotate3d_cfg or {})
            ...
            t_r3 = _pnow() if perf_enabled else None
            rotate3d_mode7_ctx = engine_mod.apply_rotate3d_mode7(
                rotate3d_tmp,
                bg,
                cam_wx,
                cam_wy,
                ...
                float(rotate3d_current),
                mode7_cfg,
                ref_forward=float(cam_back),
                player_wx=px_w,
                player_wy=py_w,
            )
            render_surf.blit(rotate3d_tmp, (0, 0))
            if perf_enabled and t_r3 is not None:
                _padd("rotate3d_bg", _pnow() - t_r3)
            if isinstance(rotate3d_mode7_ctx, dict):
                y_transform = None
                x_scale_fn = None
                rotate3d_horizon = int(rotate3d_mode7_ctx.get("horizon", 0))
                rotate3d_mode7_ctx["player_wx"] = px_w
                rotate3d_mode7_ctx["player_wy"] = py_w
        ...
        try:
            render_xform_for_input = {
                ...
                "rotate3d_on": bool(rotate3d_active),
                "rotate3d_strength": float(rotate3d_current),
                "rotate3d_mode7": bool(rotate3d_mode7_ctx),
                "mode7_ctx": rotate3d_mode7_ctx,
                "screen_w": int(CONFIG.get("WIDTH", 320) or 320),
                "screen_h": int(CONFIG.get("HEIGHT", 240) or 240),
            }
        except Exception:
            pass
        ...
        scr_h = int(CONFIG.get("HEIGHT", 480) or 480)
        _r3_draw_kw = {}
        if rotate3d_active and isinstance(rotate3d_mode7_ctx, dict):
            _r3_draw_kw = {
                "mode7_ctx": rotate3d_mode7_ctx,
                "view_w": scr_w,
            }
        ...
        def _rough_screen_xy(ent):
            try:
                ...
                return None
            try:
                return _player_feet_screen_xy_like_draw(
                    wx, wy, cam_draw_x, cam_draw_y, z, y_transform, x_offset_fn, **_r3_draw_kw
                )
            except Exception:
                try:
                ...
                for item in pool:
                    ...
                    if isinstance(item, _FieldItemClass):
                        item.draw(
                            ...
                            y_transform=y_transform,
                            x_offset_fn=x_offset_fn,
                            sprite_perspective_q=None if rotate3d_active else (float(f_q) if callable(y_transform) else None),
        ...
        # --- 맵 ambient 물결 타일 (배경 위 · 캐릭터/활동 아래) ---
        # field.wave_tiles: 공유 프레임 + 뷰포트 컬링 (오브젝트 다수 배치 대비 저부하)
        if getattr(wave_fx, "enabled", False):
            t_wv = _pnow() if perf_enabled else None
            try:
                wave_fx.update_and_draw(
                    render_surf,
                    dt_visual_sec,
                    cam_origin_x=float(cam_draw_x),
                    cam_origin_y=float(cam_draw_y),
                    zoom=float(z),
                    map_size=(int(map_bg_w), int(map_bg_h)),
                    y_transform=y_transform,
                    x_offset_fn=x_offset_fn,
                    f_q=float(f_q) if callable(y_transform) else 1.0,
                    mode7_ctx=rotate3d_mode7_ctx if rotate3d_active else None,
                )
            except Exception:
                pass
            if perf_enabled and t_wv is not None:
                _padd("wave_tiles", _pnow() - t_wv)
        ...
        # 징검다리: 활동 시작 전에도 같은 그리드로 연꽃잎 표시 (오브젝트 중복 배치 없음)
        if str(_aid_u or "") != "lotus_cross":
            try:
                from activities.lotus_cross import draw_lotus_cross_idle

                draw_lotus_cross_idle(_fa_under, map_id, flow.world_data)
            except Exception:
                pass
        if field_activities.is_active:
            try:
        ...
        def _ysort_y(ent):
            try:
                ...
                return 0.0
            # Mode7: 화면 feet Y(멀리=작음)로 정렬 — 시선 회전 후에도 앞/뒤가 맞음
            if isinstance(rotate3d_mode7_ctx, dict):
                try:
                    return float(engine_mod.rotate3d_mode7_sort_key(wx, wy, rotate3d_mode7_ctx))
                except Exception:
                    pass
            y = wy
        ...
        # 2. 통합 정렬
        # - 1순위: layer
        # - 2순위: Mode7이면 화면 깊이(feet Y), 아니면 월드 Y
        ...
        sprite_perspective_q = float(f_q) if callable(y_transform) and (not rotate3d_active) else None
        ...
        if ui.show_mask:
            t0 = _pnow() if perf_enabled else None
            # 3D_ROTATE(Mode7) 활성 시: 배경과 같은 시점·원근으로 마스크도 샘플링
            if rotate3d_active and isinstance(rotate3d_mode7_ctx, dict) and mask is not None and rotate3d_cfg is not None:
                try:
                    lw_m = int(rotate3d_mode7_ctx.get("w", CONFIG.get("WIDTH", 640)) or 640)
                    lh_m = int(rotate3d_mode7_ctx.get("h", CONFIG.get("HEIGHT", 480)) or 480)
                except Exception:
                    lw_m, lh_m = 640, 480
                lw_m = max(1, int(lw_m))
                lh_m = max(1, int(lh_m))
                if (
                    rotate3d_mask_tmp is None
                    or rotate3d_mask_tmp.get_width() != lw_m
                    or rotate3d_mask_tmp.get_height() != lh_m
                ):
                    rotate3d_mask_tmp = pygame.Surface((lw_m, lh_m))
                engine_mod.apply_rotate3d_mode7(
                    rotate3d_mask_tmp,
                    mask,
                    float(rotate3d_mode7_ctx.get("cam_x", 0.0)),
                    float(rotate3d_mode7_ctx.get("cam_y", 0.0)),
                    float(rotate3d_mode7_ctx.get("cam_angle", 0.0)),
                    float(rotate3d_current),
                    rotate3d_cfg,
                    ctx=rotate3d_mode7_ctx,
                    clear_color=(0, 0, 0),
                    fill_sky=False,  # 마스크는 하늘/바닥 채움 없이 검정만
                )
                try:
                    rotate3d_mask_tmp.set_colorkey((0, 0, 0))
                    rotate3d_mask_tmp.set_alpha(120)
                except Exception:
                    pass
                render_surf.blit(rotate3d_mask_tmp, (0, 0))
            else:
        ...
        if click_feedback:
            ...
            if elapsed < 1000: # 1초 동안 표시
                ...
                if (elapsed // 200) % 2 == 0:
                    ...
                    try:
                        fx, fy = _player_feet_screen_xy_like_draw(
                            wx0, wy0, cam_draw_x, cam_draw_y, zmk, y_transform, x_offset_fn, **_r3_draw_kw
                        )
                    except Exception:
            ...
            if zones:
                ...
                if img is not None:
                    ...
                    for z0 in zones:
                        ...
                        zmk = max(1e-6, float(zmk))
                        fx, fy = _player_feet_screen_xy_like_draw(
                            cxw, cyw, cam_draw_x, cam_draw_y, zmk, y_transform, x_offset_fn, **_r3_draw_kw
                        )
        ...
        if (
            ...
            for _dist, ent, (cxw, cyw) in ep_candidates:
                ...
                try:
                    ...
                    eimg = eframes[0]
                fx, fy = _player_feet_screen_xy_like_draw(
                    cxw, cyw, cam_draw_x, cam_draw_y, zmk_ep, y_transform, x_offset_fn, **_r3_draw_kw
                )
                try:
        ...
        if field_activities.is_active:
            try:
                _aid = None
                try:
                    _aid = getattr(getattr(field_activities, "_session", None), "activity_id", None)
                except Exception:
                    _aid = None
        ...
        if field_activities.is_active:
            try:
                _aid2 = None
                try:
                    _aid2 = getattr(getattr(field_activities, "_session", None), "activity_id", None)
                except Exception:
                    _aid2 = None
        ...
        try:
            ...
            head_ctx = None
        ev_mgr.draw_ui_overlays(render_surf, head_ctx, layer="chrome")
        ...
        if ev_mgr.fade_alpha > 0:
            fc = getattr(ev_mgr, "fade_color", None) or (0, 0, 0)
            try:
                rgb = (
                    int(max(0, min(255, int(fc[0])))),
                    int(max(0, min(255, int(fc[1])))),
                    int(max(0, min(255, int(fc[2])))),
                )
            except Exception:
                rgb = (0, 0, 0)
            fade_overlay_surf.fill(rgb)
...
if __name__ == "__main__":
    try:
        ...
        try:
            _diag(f"UNCAUGHT last_cp={_diag_cp} loop_i={_diag_loop_i}\n{tb}")
        except Exception:
            pass
        log_line("=== crash ===")
